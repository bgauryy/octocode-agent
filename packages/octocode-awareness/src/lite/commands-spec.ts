/**
 * Machine-readable command contract for Awareness Lite — the SINGLE SOURCE OF
 * TRUTH for the CLI verbs and their parameters. Hosts embedding this package
 * (e.g. the Pi extension) import `AWARENESS_COMMANDS` to GENERATE typed,
...
export const AWARENESS_COMMANDS: readonly CommandGroup[] = [
  ...
  },
  {
    ...
    actions: [
      ...
      { action: 'inbox', summary: 'read messages addressed to you', needsAgentId: true, params: [{ name: 'topic', flag: 'topic', type: 'string', description: 'Topic filter.' }, { name: 'includeRead', flag: 'include-read', type: 'boolean', description: 'Include already-read messages.' }, { name: 'limit', flag: 'limit', type: 'integer', min: 1, max: 100, description: 'Max messages.' }] },
      { action: 'read', summary: 'mark a message read', needsAgentId: true, params: [{ name: 'messageId', flag: 'message-id', type: 'string', required: true, description: 'Message id.' }] },
    ],
  },
  {

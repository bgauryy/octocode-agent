...
import { isEmbeddingEnabled,openAwarenessLite } from './index.js';
 ...
 * as before; `runCli`/`execCli` swap it so an in-process host (e.g. the Pi
 * extension importing this package as a library) can capture the same JSON a
 ...
 * once in `dispatch.ts`, so the CLI never names an `AwarenessLite` method or its
 ...
 * printed, so a host can embed Awareness Lite as a library and reuse the exact

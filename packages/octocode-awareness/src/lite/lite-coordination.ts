...
import { LitePlansTasks } from './lite-plans-tasks.js';
import { handoffFromRow,HandoffRow,id,lockFromRow,LockRow,now,required,sleepMs,splitFiles,workPresenceFromRow,WorkPresenceRow } from './lite-shared.js';

export abstract class LiteCoordination extends LitePlansTasks {
  ...
  listLocks(): Lock[] {
    this.pruneExpiredLocks();
    const rows = this.db.prepare('SELECT * FROM locks WHERE workspace_path = ? ORDER BY file_path ASC').all(this.workspace) as unknown as LockRow[];

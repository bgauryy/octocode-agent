...
describe('fileLock', () => {
  it('rejects late renewal after expired exclusivity is reacquired by another run', () => {
    ...
    try {
      ...
      expect(() => fileLock(db, { type: 'renew', agentId: 'agent-a', runId: first.run_id }))
        .toThrow(/conflict/i);
  ...
  it('closes every advisory presence when the final lock release makes a run pending', () => {
    ...
    try {
      ...
      const attached = startWork(db, {
        agentId: 'agent-a', runId: locked.run_id, workspacePath: dir, targetFiles: ['src/b.ts'],
      });
      if (!attached.ok) throw new Error('advisory attachment failed');

      releaseFileLock(db, { agentId: 'agent-a', runId: locked.run_id, status: 'PENDING' });
      expect(db.prepare('SELECT status FROM task_runs WHERE run_id = ?').get(locked.run_id))
        .toEqual({ status: 'PENDING' });
      expect(db.prepare('SELECT COUNT(*) AS count FROM run_files WHERE run_id = ? AND ended_at IS NULL')
        .get(locked.run_id)).toEqual({ count: 0 });
    } finally { cleanup(); }
  });
  ...
  it('locks, reports status, renews, and releases by task id', () => {
    ...
    try {
      ...
      if (!locked.ok || locked.type !== 'lock') throw new Error('lock failed');
      expect(locked.run_id).toMatch(/^run_/);
      expect(locked.locks.map((lock) => lock.path)).toEqual([canonicalizePath(join(dir, 'src/a.ts'))]);
      expect(locked.locks[0]?.expires_at).toBeTruthy();
      ...
      expect(status.locks[0]!.run_id).toBe(locked.run_id);

      const beforeWrongRenew = db.prepare('SELECT heartbeat_at, expires_at FROM run_files WHERE run_id = ?')
        .get(locked.run_id);
      expect(() => fileLock(db, { type: 'renew', agentId: 'agent-b', runId: locked.run_id, ttlMs: 60 * 60_000 }))
        .toThrow(/belongs to agent-a/);
      expect(db.prepare('SELECT heartbeat_at, expires_at FROM run_files WHERE run_id = ?').get(locked.run_id))
        .toEqual(beforeWrongRenew);

      const renewed = fileLock(db, { type: 'renew', agentId: 'agent-a', runId: locked.run_id, ttlMs: 60 * 60_000 });
      expect(renewed.type).toBe('renew');
      if (renewed.type !== 'renew') throw new Error('renew failed');
      expect(renewed.renewed).toBe(true);
      expect(Date.parse(renewed.expires_at!)).toBeGreaterThanOrEqual(Date.parse(locked.locks[0]!.expires_at!));

      const released = fileLock(db, { type: 'release', agentId: 'agent-a', runId: locked.run_id, status: 'PENDING' });

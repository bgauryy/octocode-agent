...
describe('AwarenessStore', () => {
  ...
  it('stores, recalls, and describes local memory', () => {
    ...
    expect(aw.schema().commands.memory).not.toContain('delete --memory-id');
  ...
  it('projects expired task claims as open without mutating stored state', () => {
    ...
    try {
      db.prepare('UPDATE tasks SET lease_expires_at = ? WHERE task_id = ?')
        .run(new Date(Date.now() - 1000).toISOString(), task.taskId);
      expect(aw.listTasks({ planId: plan.planId })[0]).toMatchObject({ status: 'OPEN', agentId: null, leaseExpiresAt: null, verificationMessage: null });
      expect(db.prepare('SELECT status, agent_id AS agentId FROM tasks WHERE task_id = ?').get(task.taskId))
        .toEqual({ status: 'CLAIMED', agentId: 'agent-a' });
    } finally {
      db.close();
    }
  });

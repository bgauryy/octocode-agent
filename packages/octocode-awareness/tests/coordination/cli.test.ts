...
describe('isCliEntrypoint', () => {
  it('accepts symlinked bin paths used by npx and package-manager shims', async () => {
    ...
    const linkCli = join(workspace, 'octocode-awareness');
...
describe('runCli', () => {
  ...
  it('runs the agent/message JSON flow', () => {
    ...
    stdout = '';
    expect(runCli(['message', 'read', '--workspace', workspace, '--agent-id', 'agent-b'])).toBe(0);
    const inbox = jsonOut<Array<{ messageId: string; text: string; readAt: string | null }>>();
    expect(inbox).toMatchObject([{ messageId: message.messageId, text: 'please review README' }]);

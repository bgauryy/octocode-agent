...
import { openAwarenessLite, type AwarenessLite } from '../../src/lite/index.js';
...
describe('agent naming', () => {
  it('generates funny host-tagged names and detects hosts from env', async () => {
    const { detectAgentHost, generateAgentName } = await import('../../src/lite/index.js');
  ...
  it('creates the shared agent/session tables in the same file as lite tables', () => {
    ...
    try {
      ...
      // agent-owned tables (from @octocodeai/octocode-shared) coexist with lite's.
      expect(names).toEqual(expect.arrayContaining(['agent_sessions', 'octocode_meta', 'plans', 'locks']));
...
describe('AwarenessLite cross-workspace isolation (single global file)', () => {
  let root: string;
  let dbPath: string;
  let repoA: string;
  let repoB: string;
  ...
  beforeEach(async () => {
    ...
    a = openAwarenessLite({ workspace: repoA, dbPath });
    b = openAwarenessLite({ workspace: repoB, dbPath });
  });

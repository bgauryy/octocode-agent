...
When changing this skill, edit the package-local `packages/octocode-awareness/skills/octocode-rfc-generator/` copy (canonical source), then run `node packages/octocode-pi-extension/scripts/build.mjs` to sync the pi-extension mirrors. Before reporting done, run `node scripts/eval-rfc.mjs --self-test` and the smoke prompts in `evals/prompts.md`, and run the skill linter until it reports 0 errors.

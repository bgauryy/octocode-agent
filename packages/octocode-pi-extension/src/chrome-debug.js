...
export function restrictedFetch(input, init) {
...
// ─── CDP HTTP discovery ───────────────────────────────────────────────────────
async function cdpHttp(port, urlPath, method = 'GET') {
    const url = `http://127.0.0.1:${port}${urlPath}`;
    if (!isLocalhost(url))
        throw new Error(`[SANDBOX] CDP HTTP blocked: ${url}`);
    const res = await _origFetch(url, { method, signal: AbortSignal.timeout(5000) });
    if (!res.ok)
        throw new Error(`CDP HTTP ${res.status} for ${urlPath}`);
    return res.json();
}
export async function getVersion(port) {
    return cdpHttp(port, '/json/version');
}
export async function getTargets(port) {
    return cdpHttp(port, '/json');
}
async function openTab(port, url) {
    return cdpHttp(port, `/json/new?${encodeURIComponent(url)}`, 'PUT');
}
 ...
 */
export function createCdpSession(wsUrl, targetInfo, defaultTimeoutMs = 60_000, signal, logPath) {
    return new Promise((resolveSession, rejectSession) => {
        ...
        ws.onopen = () => {
            const session = {
                ...
                send(method, params = {}, sessionId, timeoutMs = defaultTimeoutMs) {
                    if (isClosed)
                        return Promise.reject(new Error('Session already closed'));
                    if (signal?.aborted)
                        return Promise.reject(new Error('Session aborted'));
        ...
        ws.onmessage = (evt) => {
            let msg;
            try {
                msg = JSON.parse(typeof evt === 'string' ? evt : evt.data);
            }
            catch {
                return;
            }
            ...
            if (msgId !== undefined && pending.has(msgId)) {
                ...
                handlers.get(event)?.forEach((h) => {
                    ...
                    catch { /* handler errors don't crash session */ }
                });
        ...
        ws.onerror = (e) => {
            ...
            if (!isClosed)
                rejectSession(new Error(`WebSocket error: ${msg}`));
        };
        ws.onclose = () => {
...
// ─── Target selection ─────────────────────────────────────────────────────────
export async function selectTarget(port, opts) {
    ...
    if (newTab) {
        ...
        if (!tab.webSocketDebuggerUrl)
            throw new Error(`No WebSocket URL for new tab at ${newTab}`);
        return {
    ...
    if (targetId) {
        ...
        if (!t.webSocketDebuggerUrl)
            throw new Error(`No WebSocket URL for target ${targetId}`);
    ...
    if (targetUrl) {
        ...
        if (!t.webSocketDebuggerUrl)
            throw new Error(`No WebSocket URL for target ${t.id}`);
    ...
    if (targetType) {
        ...
        if (!t.webSocketDebuggerUrl)
            throw new Error(`No WebSocket URL for target type ${targetType}`);
    ...
    const t = pages[0];
    if (!t.webSocketDebuggerUrl)
        throw new Error('No WebSocket URL for first page target');
    return { wsUrl: t.webSocketDebuggerUrl, targetInfo: t, via: 'first-page' };
}
// ─── Chrome 136 detection + launch ───────────────────────────────────────────
function parseChromeVersion(browserString) {
...
export function findChromePath() {
    ...
    throw new Error('Chrome not found. Install Google Chrome or Chromium, or start Chrome manually with ' +
        '--remote-debugging-port=9222 --user-data-dir=~/.octocode/chrome-debug/profile');
}
export function getDefaultToolUserDataDir() {
...
export function getPortUserDataDir(port) {
...
export async function launchChrome(opts) {
    ...
    if (headless) {
        args.push('--headless=new', '--disable-gpu', '--disable-dev-shm-usage', '--hide-scrollbars', '--mute-audio');
...
export function writeSessionMeta(sessionFile, meta) {
    fs.mkdirSync(path.dirname(sessionFile), { recursive: true });
    fs.writeFileSync(sessionFile, JSON.stringify(meta, null, 2) + '\n', { mode: 0o600 });
}
// ─── Infer identity ───────────────────────────────────────────────────────────
export async function inferIdentity(session, mode, version, userDataDir) {
    const identity = {
        mode,
        userDataDir,
    ...
    try {
        ...
        const pageInfo = JSON.parse(uaResult['result']?.['value'] ?? '{}');
...
// ─── Screenshot writer ────────────────────────────────────────────────────────
export function buildScreenshotFilename(scheme, urlOrSlug, ext) {
    const now = new Date();
...
export async function captureScreenshot(session, opts) {
    ...
    // Screenshot
    const ssParams = { format };
    if (quality !== undefined && format !== 'png')
        ssParams['quality'] = quality;
    if (clip)
        ssParams['clip'] = clip;
    if (fullPage) {
...
// ─── CDP retry marker ─────────────────────────────────────────────────────────
export function buildRetryMarker(error, method) {
...
export function isCdpError(error) {
...
export async function connectToChrome(opts) {
    const { port = 9222, targetId, targetUrl, targetType, newTab, launch = false, headless = false, timeoutMs = 60_000, signal, workspaceCwd, } = opts;
    ...
    if (!version && launch) {
        ...
        if (!version)
            throw new Error(`Chrome did not start within 20s on port ${port}`);
    }
    else if (!version) {
    ...
    if (chromeMajor >= 136 && mode === 'launched') {
        ...
        if (defaultDirs.includes(path.resolve(userDataDir))) {
            throw new Error(`Chrome ≥${chromeMajor} blocks remote debugging with the OS default profile.\n` +
    ...
    const cdpLogPath = process.env['OCTOCODE_CDP_DEBUG'] === '1'
        ? path.join(getSessionDir(workspaceCwd ?? path.join(os.homedir(), '.octocode'), port), 'cdp-events.jsonl')
        : undefined;
    if (cdpLogPath) {
...
// ─── Cleanup ──────────────────────────────────────────────────────────────────
export async function cleanupConnection(session, keepTab, killLaunched = false) {
    const s = session;
    const openedTabId = s['_openedTabId'];
    const port = s['_port'];
    const launchedPid = s['_launchedPid'];
    ...
    if (killLaunched && launchedPid !== undefined) {
        ...
        const launchedUserDataDir = s['_launchedUserDataDir'];

...
export function rehydrateSession(
  ...
  if (reason === 'fork') {
    pendingBySession.delete(artifact.identity.sessionKey);
  ...
  const openContinuity = dependencies.openContinuity ?? ((value: string) => openAwareness({ workspace: value }));
...
export function consumeValidatedRehydration(
  ...
  for (const checkpoint of pending.ledger.segments) {
    ...
    if ((checkpoint.tokenBudget !== undefined && tokens > checkpoint.tokenBudget)
      || estimatedTokens + tokens > (options.totalTokenBudget ?? 50_000)) {
    ...
    if (estimatedTokens + projectedTokens > (options.totalTokenBudget ?? 50_000)) {
      overBudget.push(checkpoint.id);
      continue;
    }

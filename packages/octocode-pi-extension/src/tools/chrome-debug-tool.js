...
// ─── Registration ─────────────────────────────────────────────────────────────
export function registerChromeDebugTool(pi, Type, registeredToolNames, registerFn, _notify) {
    registerFn(pi, registeredToolNames, {
        ...
        parameters: Type.Object({
            ...
            action: Type.Optional(Type.Unsafe({
                type: 'string',
                enum: [...ACTIONS],
                description: 'Verb within the scheme. Most schemes default to observe.',
            })),
            // Navigation / target
            url: Type.Optional(Type.String({ description: 'URL to navigate to before running the scheme recipe.' })),
            selector: Type.Optional(Type.String({ description: 'CSS selector for DOM-focused schemes.' })),
            expression: Type.Optional(Type.String({ description: 'JavaScript expression to evaluate (action: eval or live-page).' })),
            interact: Type.Optional(Type.Object({
            ...
            // Raw action
            method: Type.Optional(Type.String({ description: 'CDP Domain.method for scheme:"raw". Example: "Network.getCookies".' })),
            params: Type.Optional(Type.Unsafe({ type: 'object', additionalProperties: true, description: 'CDP params object for scheme:"raw". Example: {"urls":["https://example.com"]} for Network.getCookies, {"query":"button"} for DOM.performSearch.' })),
            sessionId: Type.Optional(Type.String({ description: 'Route to a worker/iframe CDP session.' })),
            // Screenshot
            format: Type.Optional(Type.Unsafe({
                type: 'string',
            ...
            fullPage: Type.Optional(Type.Boolean({ description: 'Capture full page height (captureBeyondViewport).' })),
            // Emulate
            ...
            // Session / lifecycle
            durationMs: Type.Optional(Type.Integer({ description: 'Observation window in ms for monitor/observe schemes. Default: 5000.' })),
            timeoutMs: Type.Optional(Type.Integer({ description: 'Per-call CDP timeout in ms. Default: 60000.' })),
            port: Type.Optional(Type.Integer({
                default: 9222,
                description: 'Chrome remote debugging port. Default: 9222.',
            })),
            targetId: Type.Optional(Type.String({ description: 'Attach to a specific CDP target by ID.' })),
            targetUrl: Type.Optional(Type.String({ description: 'Attach to a target whose URL contains this substring.' })),
            targetType: Type.Optional(Type.String({ description: 'Attach to a target of this type (page, worker, …).' })),
            newTab: Type.Optional(Type.String({ description: 'Open a new tab at this URL.' })),
            keepTab: Type.Optional(Type.Boolean({ description: 'Keep the target alive after the call. Default: true.' })),
            launch: Type.Optional(Type.Boolean({
                description: 'Launch Chrome if not already running on the port. ' +
                    'Always uses a non-default --user-data-dir (Chrome ≥136 requirement).',
            })),
            headless: Type.Optional(Type.Boolean({ description: 'Launch Chrome headless. Default: false (visible).' })),
            stealth: Type.Optional(Type.Boolean({ description: 'Inject stealth evasions before navigation: patches navigator.webdriver, window.chrome, plugins, vendor, hardwareConcurrency, permissions, WebGL. Use on sites with bot detection.' })),
            bypassCSP: Type.Optional(Type.Boolean({ description: 'Bypass Content-Security-Policy before script injection. Required for scheme:"inject" on CSP-protected sites.' })),
            scriptSource: Type.Optional(Type.String({ description: 'JavaScript source to inject via scheme:"inject"/"raw" (Page.addScriptToEvaluateOnNewDocument). Runs before any page script. Preferred over nesting in params to avoid JSON escaping fragility.' })),
            scriptFile: Type.Optional(Type.String({ description: 'Absolute path to a local .mjs file whose exported *SCRIPT constant (or full text) is injected. Avoids inline string escaping. Example: "/abs/path/stealth-inject.mjs".' })),
            depth: Type.Optional(Type.Integer({ description: 'Max results to return for scheme:"scrape" (default 50) or AX tree depth for scheme:"accessibility" (default -1 = full).' })),
            xpath: Type.Optional(Type.String({ description: 'XPath expression for scheme:"scrape". Evaluated alongside selector.' })),
        ...
        async execute(_toolCallId, rawParams, signal, _onUpdate, ctx) {
            ...
            try {
                ...
                throw new Error(`[CHROME_DEBUG_ERROR] ${err.message ?? String(err)}`);
            }
            const { session, version, metadata, screenshotDir } = connection;
            // Emit SESSION line
            const identity = metadata.identity;
            const sessionLine = `[SESSION] mode=${metadata.mode} browser=${version.Browser ?? 'unknown'} ` +
                `tab=${identity?.tabHost ?? '?'}${identity?.tabPath ?? ''} ` +
                `cookies=${(identity?.cookieNames ?? []).length} names`;
            setStatus(ctx, `⧗ chromeDebug · ${scheme}/${action} · target ${session.targetInfo.id.slice(0, 8)}`);
        ...
        renderResult(result, opts, theme) {
            ...
            return makeRenderer((w) => [
                ...
                ...lines.map((l) => truncateToWidth(l.startsWith('[FINDING]')

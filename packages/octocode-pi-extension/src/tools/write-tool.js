...
function resolveWritePath(filePath, cwd = process.cwd()) {
...
export function registerWriteTool(pi, Type) {
    ...
    pi.registerTool?.({
        ...
        description: 'Octocode custom write tool. Replaces Pi built-in write with the same create/overwrite semantics plus Octocode path-guard (working directory, home, OS temp, ALLOWED_PATHS) and post-write read-state recording for the edit stale-check. Prefer edit for surgical changes to existing files.',
        ...
        renderCall(args, theme) {
            ...
            const suffix = theme?.fg('dim', `${filePath} · ${lines} line${lines === 1 ? '' : 's'}`) ??

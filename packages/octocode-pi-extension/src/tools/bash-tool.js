 ...
 */
export function extractBashWriteTargets(command, cwd) {
    const targets = [];
    const push = (raw) => {
        const cleaned = raw
            .trim()
        ...
        if (cleaned === '/dev/null' || cleaned.startsWith('/dev/fd/'))
            return;
...
function assertBashCommandAllowed(command, cwd) {
    for (const pattern of BLOCKED_COMMAND_PATTERNS) {
        if (pattern.test(command)) {
            throw new Error(`bash blocked: command matches a catastrophic pattern. ` +
...
async function runBash(command, cwd, timeoutSec, signal) {
...
export function registerBashTool(pi, Type) {
    const parameters = Type.Object({
        command: Type.String({ description: 'Bash command to execute' }),
        timeout: Type.Optional(Type.Integer({ description: 'Timeout in seconds (optional, no default timeout)' })),
    }, { additionalProperties: false });
    pi.registerTool?.({
        name: 'bash',
        label: 'bash (Octocode)',
        description: 'Octocode custom bash tool. Replaces Pi built-in bash with the same shell execution plus Octocode path-guard on redirect/tee/cp/mv write targets (cwd / home / OS temp / ALLOWED_PATHS) and a small blocklist of catastrophic commands. Prefer edit/write for ordinary file mutations; use bash for git, builds, tests, and bulk mechanical edits.',
        ...
        renderResult(result, opts, theme) {
            ...
            return makeRenderer((width) => colored.split('\n').map((line) => truncateToWidth(line, width)));
        },
    });
}

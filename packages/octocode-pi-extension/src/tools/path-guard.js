 ...
 */
export function assertPathAllowed(targetPath, cwd = process.cwd(), action = 'access') {
    const real = realpathBounded(targetPath);
    const roots = allowedRoots(cwd);
    if (roots.some((root) => isWithin(real, root)))
        return;
    throw new Error(`${action} blocked: "${targetPath}" is outside the allowed roots ` +

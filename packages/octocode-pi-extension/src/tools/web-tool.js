...
export function registerWebTool(pi, Type, registeredToolNames, registerFn) {
    registerFn(pi, registeredToolNames, {
        name: 'web',
        label: 'Web',
        description: 'Browse the live web. Pass `url` to fetch and read a page as clean text (like visiting it), ' +
        ...
        parameters: Type.Object({
            ...
            page: Type.Optional(Type.Integer({
                minimum: 1,
                maximum: 20,
                description: 'Fetch: page number for long documents (default 1). Each page is maxChars chars. Pass page: 2, 3… when the result shows truncated: true.',
            })),
            engine: Type.Optional(Type.String({
                description: 'Search: force a provider — "tavily", "serper", or "duckduckgo" (default: auto by available key).',
            })),

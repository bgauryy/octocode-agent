...
let processFactory = (command, args, options) => spawn(command, args, options);
...
export function setAgentProcessFactoryForTests(factory) {
    processFactory = factory ?? ((command, args, options) => spawn(command, args, options));
    agents.clear();
}
...
export function cleanupSpawnedAgentsForShutdown() {
    ...
    for (const record of alive)
        killAgent(record, { forceKillDelayMs: 0 });
    return alive.length;
}
function installProcessCleanupHandlers() {
    if (processCleanupHandlersInstalled || process.env[SUBAGENT_ENV_VAR] === '1')
        return;
...
function writeTempPromptFile(name, text) {
...
function pushCapped(items, item) {
    items.push(item);
    if (items.length > MAX_STORED_EVENTS)
        items.splice(0, items.length - MAX_STORED_EVENTS);
}
...
function updateLastOutput(record, message) {
    const text = extractTextFromMessage(message);
    if (text)
        record.lastOutput = text;
}
function getEventToolName(event) {
...
function recordToolStart(record, event) {
...
function recordToolEnd(record, event) {
    const toolName = getEventToolName(event);
    const toolCallId = getEventToolCallId(event);
    if (!toolName && !toolCallId)
        return;
    const call = [...record.toolCalls].reverse().find((item) => (toolCallId ? item.toolCallId === toolCallId : item.toolName === toolName) && item.status === 'running');
...
function processRpcLine(record, line) {
    ...
    if (eventType === 'tool_call' || eventType === 'tool_execution_start') {
        ...
        if (resp.success === false) {
            if (!record.error)
                record.error = resp.error ?? `RPC command failed: ${resp.command ?? 'unknown'}`;
            touch(record);
        }
    }
    ...
    else if (eventType === 'message_end' && event.message) {
...
function sendRpc(record, payload) {
...
export function spawnRpcAgent(params, ctx) {
    ...
    proc.on('close', (code, signal) => {
        if (stdoutBuffer.trim())
            processRpcLine(record, stdoutBuffer);
...
function summarizeAgent(record) {
...
function getAgent(agentId) {
    ...
    if (!id)
        throw new Error('AgentMessage requires agentId for all actions except action:"list". '
            + 'Use action:"list" to see all active agents.');
    const record = agents.get(id);
    if (!record)
        throw new Error(`No agent found with id: ${id.slice(0, 16)}${id.length > 16 ? '\u2026' : ''}. `
            + `Use action:"list" to see all active agents (${agents.size} registered).`);
    return record;
}
function waitForAgent(record, timeoutMs) {
...
function renderAgentResult(records, header) {
    const summaries = records.map(summarizeAgent);
    const lines = [`${header} (${records.length}):`];
...
function renderSingleAgentResult(record, header) {
    ...
    if (toolSummary)
        contentParts.push(`tools: ${toolSummary}`);
    if (output.text)
        contentParts.push('', output.text);
    if (output.truncated)
        contentParts.push(`\u2026 output truncated (${output.omittedChars} chars hidden; full content in details)`);
    return {
...
function killAgent(record, opts = {}) {
    touch(record, 'killed');
    try {
...
export function registerAgentTools(pi, Type, registeredToolNames, registerFn) {
    if (process.env[SUBAGENT_ENV_VAR] === '1')
        return;
    installProcessCleanupHandlers();
    const resourceModeSchema = stringEnumSchema(Type, ['lean', 'octocode', 'default'], 'Worker resource loading. lean disables extensions/skills/prompts/themes; octocode loads this extension explicitly; default uses Pi discovery.');
    const actionSchema = stringEnumSchema(Type, ['list', 'status', 'send', 'steer', 'followUp', 'wait', 'kill', 'abort'], 'AgentMessage action. abort sends Pi RPC abort (graceful interrupt without killing the process).');
    registerFn(pi, registeredToolNames, {
        name: 'spawnAgent',
        label: 'Agent: Spawn Parallel Worker',
        description: 'Spawn a separate background Pi worker process over RPC. Returns immediately with an agentId; use AgentMessage to inspect, send follow-ups, wait, or kill. Workers can run in parallel but share the selected cwd and environment-backed services.',
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            const record = spawnRpcAgent(params, ctx);
    ...
    registerFn(pi, registeredToolNames, {
        ...
        description: 'Manage spawned agents. Actions: list, status, send, steer, followUp, wait, kill, abort. Use this after spawnAgent to coordinate parallel workers.',
        ...
        parameters: Type.Object({
            ...
            streamingBehavior: Type.Optional(stringEnumSchema(Type, ['steer', 'followUp'], 'For action:"send", how to queue if the worker is currently streaming. Defaults to followUp only while the worker is already running.')),
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            if (action === 'wait') {
                if (ctx?.hasUI)
                    ctx.ui?.setStatus?.('agent-wait', `\u29D7 Waiting for \u201C${record.name}\u201D\u2026`);
                try {
                    await waitForAgent(record, Number(params['timeoutMs'] ?? 300000));
                }
                finally {
                    if (ctx?.hasUI)
                        ctx.ui?.setStatus?.('agent-wait', '');
                }
            ...
            if (!message)
                throw new Error(`AgentMessage action:${action} requires message.`);
            ...
            if (!isProcessAlive(record)) {
                throw new Error(`AgentMessage action:${action} cannot reach agent "${record.name}" — it has ${record.status} (process exited). Spawn a fresh worker.`);
            }

...
export async function renderMcpManagerPage(ctx?: PiContext, actionToken = '', piSkills?: SkillInfo[], commands: readonly PiCommand[] = []): Promise<string> {
  ...
  const bodyHtml = `<style>
    ...
    <div class="section-heading" id="agent-context"><div><h2>Agent context</h2><p>What the next agent call will receive.</p></div></div><section><h2>Agent prompt catalog</h2>
      ...
      <p class="muted">Mode source: <code>OCTOCODE_COMPACT_MCP</code> (${artifacts.mode === 'compact' ? 'enabled' : 'unset/disabled'}) · mcp.md: ${escapeHtml(artifacts.guideState)}${artifacts.capturedAt ? ` · captured ${escapeHtml(artifacts.capturedAt)}` : ''}</p>

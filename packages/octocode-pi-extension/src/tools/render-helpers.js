 ...
 */
export function makeRenderer(lines) {
    return {
        render: (width = 80) => lines(width).map((line) => truncateToWidth(line, width)),
        invalidate() { },
    };
}
export function singleLineRenderer(rawLine) {
 ...
 */
export function buildToolCallSummary(toolName, args) {
    const a = (args ?? {});
 ...
 */
export function buildResultStats(toolName, details) {
    ...
    if (toolName === 'localFindFiles') {
        ...
        for (const r of results) {
            ...
            if (Array.isArray(data.entries))
                fileCount += data.entries.length;
            else if (typeof data.totalEntries === 'number')
                fileCount += data.totalEntries;
        }
    ...
    if (toolName === 'lspGetSemantics') {
        ...
        for (const r of results) {
            ...
            if (data.location && typeof data.location === 'object') {
                ...
                if (uri)
                    paths.push(`${basename(uri.replace(/\?.*$/, ''))}:${loc.line ?? ''}`);
            }
            // references: data.references[]
            if (Array.isArray(data.references))
                refCount += data.references.length;
            if (Array.isArray(data.symbols))
                refCount += data.symbols.length;
        }
        return {
            queryCount,
    ...
    if (toolName === 'npmSearch') {
        ...
        for (const r of results) {
            ...
            if (name)
                paths.push(version ? `${name}@${version}` : name);
        }
...
export function buildOctocodeRenderCall(toolName, args, theme) {
...
export function buildOctocodeRenderResult(toolName, result, opts, theme) {
    if (opts.isPartial) {
    ...
    return makeRenderer((width) => {
        ...
        if (omitted > 0) {
            out.push(truncateToWidth(theme?.fg('muted', `… ${omitted} more line${omitted === 1 ? '' : 's'} hidden (full output available to agent)`) ??
                `… ${omitted} more lines hidden`, width));
        }
        return out;
    });
}

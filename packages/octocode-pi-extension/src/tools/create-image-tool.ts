...
export function registerCreateImageTool(
  ...
  registerFn(pi, registeredToolNames, {
    ...
    renderResult(result: ToolCallResult, opts: { expanded?: boolean; isPartial?: boolean }, theme?: PiTheme, context?: RenderContext) {
      ...
      const details = (result.details ?? {}) as { base64?: string; mimeType?: string; name?: string; bytes?: number; savedPath?: string };
      if (!details.base64 || !details.mimeType) return base;
      const cacheKey = details.savedPath ?? details.name ?? 'createImage';
      return {
        render: (width = 80) => [
          ...base.render(width),
          ...buildImageLinesFromData(context, cacheKey, details.base64!, details.mimeType!, width, {
            theme,
            name: details.name,
            bytes: details.bytes,
          }),

...
// ─── Tool registration ─────────────────────────────────────────────────────────
export function registerBrowserAgentTool(pi, Type, registeredToolNames, registerFn, notify) {
    ...
    registerFn(pi, registeredToolNames, {
        ...
        parameters: Type.Object({
            task: Type.String({
                description: 'Natural language task. Examples: "analyze security headers and cookies", ' +
            ...
            url: Type.Optional(Type.String({ description: 'URL to navigate to and analyze.' })),
            port: Type.Optional(Type.Integer({ description: 'Chrome remote debug port (default 9222).' })),
            launch: Type.Optional(Type.Boolean({ description: 'Launch Chrome if not running (default false).' })),
            headless: Type.Optional(Type.Boolean({ description: 'Headless Chrome when launching (default true).' })),
            model: Type.Optional(Type.String({ description: 'Model override from `pi -ne --list-models [search]` to include in the returned spawnAgent config.' })),
            runNow: Type.Optional(Type.Boolean({
                description: 'Run initial chromeDebug analysis immediately (default true). ' +
                    'Set false to only get the spawn config without running.',
            })),
            durationMs: Type.Optional(Type.Integer({ description: 'Observation window per scheme in ms (default 5000).' })),
            workspaceCwd: Type.Optional(Type.String({ description: 'Workspace root for screenshots/session paths.' })),
        }),
                ...
                try {
                    ...
                    for (const scheme of schemes.slice(0, 4)) {
                        ...
                        try {
                            ...
                            allLines.push(`[AGENT] ${scheme} error: ${err.message.slice(0, 100)}`);
                        }
                    }
                }
                catch (err) {
                    allLines.push(`[AGENT] connect error: ${err.message}`);
                }
                finally {
                    if (conn) {

 ...
 * handles itself.
 */
export function applyFilterKey(buffer: string, keyData: string): FilterKeyResult {
...
export async function runSelectOverlay(
  ...
  return ctx.ui.custom<string | null>((tui: any, theme: PiTheme, _kb: unknown, done: (v: string | null) => void) => {
    const container = new Container();
    const heading = (theme?.fg?.('accent', theme?.bold?.('◆ ' + opts.title) ?? '◆ ' + opts.title)) ?? '◆ ' + opts.title;
    container.addChild(new Text(heading, 1, 0));

    let filter = '';
    const filterLine = enableFilter ? new Text(theme?.fg?.('dim', 'filter: ') ?? 'filter: ', 1, 0) : undefined;
    if (filterLine) container.addChild(filterLine);

    const list = new SelectList(
      opts.items.map((o) => ({ value: o.value, label: o.label, description: o.description })) as any,
      ...
      handleInput: (data: string) => {
        if (enableFilter) {
          ...
          if (next.changed) {
            ...
            filterLine?.setText?.(theme?.fg?.('dim', `filter: ${filter}`) ?? `filter: ${filter}`);
            tui?.requestRender?.();
            return;
          }
        }

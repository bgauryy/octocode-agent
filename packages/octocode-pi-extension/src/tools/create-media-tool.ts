...
import { createImageFromSvg, createImageFromHtml, renderHtmlToPdf } from './create-image-tool.js';
  ...
  registerFn(pi, registeredToolNames, {
      ...
      return executeQueryBatch({
        ...
        async execute(query, _index, _callId, batchSignal) {
          ...
          return {
            content,
            details: {
              ok: true, type: res.type, base64: res.base64, mimeType: res.mimeType, bytes: res.bytes,
              savedPath: res.savedPath, probe: res.probe, ffprobe: res.ffprobe,
            },
          };
        },
      });
    ...
    renderResult(result: ToolCallResult, opts: { expanded?: boolean; isPartial?: boolean }, theme?: PiTheme, context?: RenderContext) {
      ...
      return {
        render: (width = 80) => [
          ...base.render(width),
          ...buildImageLinesFromData(context, cacheKey, details.base64!, details.mimeType!, width, { theme, bytes: details.bytes }),
        ],

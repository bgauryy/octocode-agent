...
export function resolveUserAgent(env) {
...
export function isBlockedIp(ip) {
    ...
    if (version === 4)
        return V4_BLOCKS.some(([base, bits]) => inV4(ip, base, bits));
    if (version === 6) {
        ...
        if (groups) {
            ...
            if (embedded)
                return isBlockedIp(embedded); // recurse into V4 ranges for any embedded form
        }
        if (/^fe[89ab]/.test(lo))
...
export async function assertPublicUrl(urlStr, opts = {}) {
    ...
    for (const rec of ips) {
        ...
        if (isBlockedIp(addr))
            throw new Error(`Host ${hostname} resolves to blocked address ${addr}`);
    }
    return url;
}
/**
                    ...
                    connect: {
                        ...
                        lookup: (hostname, _options, cb) => {
                            ...
                            dns.lookup(hostname, { all: true })
                                ...
                                if (list.length === 0)
                                    return cb(new Error(`Could not resolve host: ${hostname}`), '', 0);
...
export async function safeFetch(startUrl, opts = {}) {
    const { fetchImpl = globalThis.fetch, lookup = dns.lookup, timeoutMs = 15000, maxRedirects = 5, headers = {}, signal: externalSignal, } = opts;
    ...
    const usingGlobalFetch = fetchImpl === globalThis.fetch;
    ...
    for (let hop = 0; hop <= maxRedirects; hop++) {
        ...
        if (externalSignal?.aborted)
            ac.abort(); // honor an already-aborted external signal
 ...
 */
export async function readCapped(res, maxBytes = 2_000_000, opts = {}) {
    const { signal } = opts;
    ...
    const abortPromise = signal
        ? new Promise((resolve) => {
            if (signal.aborted)
                ...
                signal.addEventListener('abort', () => resolve({ __aborted: true }), { once: true });
        })
        : null;
    let out = '';
    let total = 0;
...
function codePointOrEntity(cp, original) {
...
export function decodeEntities(str) {
    return str
        .replace(/&#(\d+);/g, (m, d) => codePointOrEntity(Number(d), m))
        .replace(/&#x([0-9a-f]+);/gi, (m, h) => codePointOrEntity(parseInt(h, 16), m))
...
export function createDeadline(opts = {}) {
    ...
    if (opts.signal) {
        if (opts.signal.aborted)
            ...
            opts.signal.addEventListener('abort', onExt, { once: true });
    }
    return {
        signal: ac.signal,
        cleanup() {
            clearTimeout(timer);
            if (opts.signal)
                opts.signal.removeEventListener('abort', onExt);
        },
    };
}
/**
...
export async function webFetch(url, opts = {}) {
    ...
    const maxBytes = Math.min(opts.maxBytes !== undefined ? Math.max(opts.maxBytes, minBytes) : minBytes, 10_000_000);
...
export async function duckDuckGoSearch(query, opts = {}) {
    ...
    try {
        ...
        if (!res.ok)
            return { error: `HTTP ${res.status} from search backend` };
...
export async function postJson(url, opts = {
    ...
    const { headers = {}, body, fetchImpl = globalThis.fetch, signal, timeoutMs = 30000 } = opts;
...
export async function serperSearch(query, opts = {}, deps = {}) {
    ...
    if (raw.answerBox)
        ...
        answer = raw.knowledgeGraph.description;
    return {
        engine: 'serper',
        query,
        answer,
...
export function pickProvider(opts = {}) {
    ...
    if (opts.engine === 'tavily' ||
        ...
        return opts.engine;
    if (normalizeApiKey(env['TAVILY_API_KEY'] ?? env['TAVILY_API_TOKEN']))
        return 'tavily';
    if (normalizeApiKey(env['SERPER_API_KEY']))
        return 'serper';
    return 'duckduckgo';
}
/**

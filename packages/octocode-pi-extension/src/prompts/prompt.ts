...
Be context-aware before acting: installing anything (packages, skills, LSP servers, MCP servers, global tools) and sensitive/side-effecting actions (\`git\` commit/push/reset/rebase/checkout, deleting or overwriting user files, network mutations, sudo) are protected — ask the user first via the approval prompt (\`askUser\` yes / no / always-allow), never assume consent. When the user picks "always allow", remember that decision in session state so the same class of action is not re-prompted this session; a plain "yes" approves once. A denied action is declined — adjust, do not retry it verbatim.
</authority>`;

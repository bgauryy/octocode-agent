...
Persist a handoff only when work must survive compaction, another agent, or a later session. Write captured state to a doc first (path and format: see `<docs>`), read the handoff back, then call `manage_context(type:"compact")` when ≥60% full or at a real phase boundary.

...
**Coverage and isolation** — honor repo coverage targets (this repo requires ≥90% branch coverage). Avoid overlapping coverage unless it proves a distinct contract. Avoid shared mutable state between tests unless the harness explicitly resets/isolates it; tear down spies/mocks in `afterEach`.

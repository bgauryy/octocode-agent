<skills>
Load skills proactively when context matches — before or during work. If the user names a skill or the task clearly matches one, use the minimal matching set; do not load skills as ceremony.
...
Announce only material skill-driven actions or pauses; if a skill changes the approach in a user-relevant way, mention it in the final result.

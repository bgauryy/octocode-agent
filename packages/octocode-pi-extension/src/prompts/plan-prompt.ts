...
export function buildPlanPrompt(goal: string): string {
  ...
  return [
    ...
    '4. Call plan(propose) with dependency-ordered, independently verifiable steps and a final real-world check. A consequential proposal must link its reviewable RFC; the user then chooses browser review, the local RFC file, or a chat TL;DR.',

...
import { createPiAwarenessBridge, getPiAwarenessAgentId, resolveDbPath, wirePiAwarenessHooks, } from './awareness.js';
import { propagateOctocodeEnv, getOctocodeHome } from './env.js';
import { OCTOCODE_DIRECT_TOOL_NAMES, DISABLED_BUILTIN_TOOL_NAMES, OVERRIDDEN_BUILTIN_TOOL_NAMES, OCTOCODE_SUPPORT_TOOL_NAMES, } from './constants.js';
...
import { shouldAppendSystemPrompt, mergeManagedAppendSystem, resolvePromptMode, composeSystemPrompt, } from './prompt.js';
import { parseSetupScope, getAppendSystemTarget, } from './utils.js';
...
export { OCTOCODE_DIRECT_TOOL_NAMES, DISABLED_BUILTIN_TOOL_NAMES, OVERRIDDEN_BUILTIN_TOOL_NAMES, OCTOCODE_SUPPORT_TOOL_NAMES, } from './constants.js';
export { PACKAGE_NAME, SYSTEM_PROMPT_MARKER, MANAGED_BLOCK_START, MANAGED_BLOCK_END, } from './constants.js';
export { getAssetPaths, getAwarenessCLIPath, readTextIfExists, listBundledSkills, getInstallSource, getCLIPath } from './assets.js';
export { shouldAppendSystemPrompt, renderSystemPromptAddendum, renderManagedAppendSystem, mergeManagedAppendSystem, resolvePromptMode, composeSystemPrompt, } from './prompt.js';
export { splitArgs, parseSetupScope, getAppendSystemTarget, truncateUserVisibleToolOutput, } from './utils.js';
...
export { cleanupSpawnedAgentsForShutdown, setAgentProcessFactoryForTests, } from './tools/agent-tools.js';
 ...
 */
export function bundledAwarenessSkillRoot() {
    const dir = path.join(getAssetPaths().skillsDir, 'octocode-awareness');
    return fs.existsSync(dir) ? dir : undefined;
}
function withAwarenessSkillRoot(options) {
    if (options.skillRoot != null)
        return options;
...
export function createAwarenessHooksAddon(options = {}) {
    const merged = withAwarenessSkillRoot(options ?? {});
    return function octocodeAwarenessHooksAddon(pi) {
        return wirePiAwarenessHooks(pi, merged);
    };
}
// ─── UI helpers ───────────────────────────────────────────────────────────────
export function getThinkingStatus(ctx, level) {
    const model = ctx?.model;
    if (!model)
        return 'thinking: unknown model';
    if (!model.reasoning)
        return `thinking: off (${model.id ?? 'model'} has reasoning:false)`;
    return `thinking: ${level ?? 'default'} (${model.id ?? 'model'})`;
}
export function applyOctocodeUi(ctx, level) {
    ...
    ui.setStatus?.('octocode-thinking', ui.theme?.fg ? ui.theme.fg('dim', thinkingStatus) : thinkingStatus);
}
...
// ─── Status / harness ────────────────────────────────────────────────────────
function formatOctocodeToolStatus() {
    return `${OCTOCODE_DIRECT_TOOL_NAMES.length} native Pi tools`;
}
export function formatStatus(baseDir) {
...
export function listExtensionHarness(baseDir) {
    return {
...
function renderExtensionHarness(baseDir) {
 ...
 */
export function disableBuiltinTools(pi) {
    ...
    try {
        ...
        if (!Array.isArray(activeTools))
            return false;
        const disabled = new Set(DISABLED_BUILTIN_TOOL_NAMES);
        ...
        if (String(error?.message ?? error).includes('Extension runtime not initialized')) {
            return false;
        }
        throw error;
    }
...
async function installAppendSystem(args, ctx) {
    ...
    const ok = await confirm(ctx, 'Install Octocode APPEND_SYSTEM.md?', `Write the managed Octocode harness block to ${targetPath}?`);
    if (!ok) {
...
// ─── Pi wiring ────────────────────────────────────────────────────────────────
async function wireOctocodePiExtension(pi, opts) {
    ...
    let derivedAwarenessAgentId = null;
    ...
    if (pi.on) {
        ...
        try {
            ...
            console.error('[octocode-pi-extension] Failed to wire awareness hooks:', error instanceof Error ? error.message : error);
        }
        ...
        pi.on('session_start', async (_event, ctx) => {
            ...
            try {
                if (disableBuiltinTools(pi)) {
                    notify(ctx, `Octocode disabled Pi built-ins (${DISABLED_BUILTIN_TOOL_NAMES.join(', ')}); use Octocode local tools instead. Overrides: ${OVERRIDDEN_BUILTIN_TOOL_NAMES.join(', ')}.`, 'info');
                }
            }
            catch (error) {
                notify(ctx, `Octocode could not disable Pi built-ins: ${error?.message ?? String(error)}`, 'warning');
            }
            try {
    ...
    pi.registerCommand('octocode-harness', {
        description: 'List every Octocode Pi extension harness surface: native tools, support tools, extension commands, CLI entry point, and skills.',
        handler: async (_args, ctx) => {

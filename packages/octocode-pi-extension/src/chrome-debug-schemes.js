...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function emit(lines, prefix, text) {
    lines.push(redactEvidence(`${prefix} ${text}`));
}
async function navigateAndWait(session, url, waitMs = 4000, signal) {
...
async function runInteract(session, interact, lines) {
    if (!interact)
        return;
    if (interact.wait) {
...
// ─── MVP Scheme: debug ────────────────────────────────────────────────────────
const debugRecipe = async ({ session, params, screenshotDir, signal }) => {
    const lines = [];
    const findings = [];
    // Enable domains
    ...
    session.on('Network.loadingFailed', ({ requestId, errorText }) => {
        const r = requests.get(requestId);
        if (!r)
            return; // skip internal Chrome requests not in our map
        const url = r.url;
        findings.push({
            priority: 3,
    ...
    try {
        ...
        pageInfo = JSON.parse(String(evalResult['result']?.['value'] ?? '{}'));
    }
...
// ─── MVP Scheme: network ──────────────────────────────────────────────────────
const networkRecipe = async ({ session, params, signal }) => {
    const lines = [];
    await session.send('Network.enable', {});
    const requests = new Map();
    const findings = [];
...
// ─── MVP Scheme: console ──────────────────────────────────────────────────────
const consoleRecipe = async ({ session, params, signal }) => {
    const lines = [];
    const findings = [];
    await session.send('Runtime.enable', {});
    await session.send('Log.enable', {});
...
// ─── MVP Scheme: dom ──────────────────────────────────────────────────────────
const domRecipe = async ({ session, params }) => {
    const lines = [];
    await session.send('DOM.enable', {});
    await session.send('Runtime.enable', {});
    if (params.url) {
    ...
    const info = JSON.parse(String(evalResult['result']?.['value'] ?? '{}'));
...
const performanceRecipe = async ({ session, params }) => {
    const lines = [];
    ...
    const paintEntries = JSON.parse(String(vitals['result']?.['value'] ?? '[]'));
...
const screenshotRecipe = async ({ session, params, screenshotDir }) => {
    const lines = [];
    await session.send('Page.enable', {});
    if (params.url) {
...
const interceptRecipe = async ({ session, params, signal }) => {
    const lines = [];
    await session.send('Network.enable', {});
...
// ─── MVP Scheme: security ─────────────────────────────────────────────────────
const securityRecipe = async ({ session, params }) => {
    const lines = [];
    const findings = [];
    await session.send('Network.enable', {});
    ...
    try {
        ...
        for (const c of cookies) {
            ...
            if (!c.secure)
                findings.push(`[FINDING] INSECURE_COOKIE: ${c.name ?? '?'} missing Secure flag`);
            if (!c.httpOnly)
                findings.push(`[FINDING] XSS_RISK_COOKIE: ${c.name ?? '?'} missing HttpOnly flag`);
        }
    }
    ...
    const headerInfo = JSON.parse(String(headerResults['result']?.['value'] ?? '{}'));
    ...
    const protoInfo = JSON.parse(String(protoResult['result']?.['value'] ?? '{}'));
    emit(lines, '[SECURITY]', `prototype pollution check: ${JSON.stringify(protoInfo)}`);
    for (const f of findings)
        lines.push(f);
    if (findings.length === 0)
        emit(lines, '[ACTION]', 'No critical cookie security issues found — check network tab for header-level CSP/HSTS');
    return { evidenceLines: lines, details: { findings, headerInfo } };
};
// ─── MVP Scheme: storage ──────────────────────────────────────────────────────
const storageRecipe = async ({ session, params }) => {
    const lines = [];
    await session.send('Network.enable', {});
    if (params.url) {
        await navigateAndWait(session, params.url, 3000);
    ...
    const storageInfo = JSON.parse(String(storageResult['result']?.['value'] ?? '{}'));
    ...
    const authKeys = (storageInfo.localStorageKeys ?? []).filter((k) => /token|auth|jwt|session|refresh/i.test(k));
...
// ─── MVP Scheme: automate ─────────────────────────────────────────────────────
const automateRecipe = async ({ session, params, signal }) => {
    const lines = [];
    await session.send('Page.enable', {});
    await session.send('Runtime.enable', {});
    ...
    if (params.expression) {
        ...
        const val = String(exprResult['result']?.['value'] ?? '');
...
const livePageRecipe = async ({ session, params }) => {
    const lines = [];
    await session.send('Runtime.enable', {});
    ...
    if (params.expression) {
        ...
        emit(lines, '[EVAL]', redactEvidence(String(exprResult['result']?.['value'] ?? '')));
    }
...
const userAuthRecipe = async ({ session, params, signal }) => {
    const lines = [];
    await session.send('Page.enable', {});
    await session.send('Network.enable', {});
    if (params.url) {
    ...
    if (params.scriptFile || params.scriptSource) {
        ...
        if (src) {
            if (method === 'Page.addScriptToEvaluateOnNewDocument')
                rawParams = { ...rawParams, source: src };
            else if (method === 'Runtime.evaluate')
                rawParams = { ...rawParams, expression: src };
        }
    }
...
// ─── accessibility ─────────────────────────────────────────────────────────────
const accessibilityRecipe = async ({ session, params }) => {
    const lines = [];
    if (params.url)
        await navigateAndWait(session, params.url);
    await session.send('DOM.enable', {});
    ...
    if (!hasMain)
        findings.push('[FINDING] AX_NO_MAIN_LANDMARK: no main landmark found');
    for (const f of findings)
        lines.push(f);
    const topRoles = Object.entries(roleCounts).sort((a, b) => b[1] - a[1]).slice(0, 8);
    emit(lines, '[A11Y]', `roles: ${topRoles.map(([r, c]) => `${r}:${c}`).join(', ')}`);
    // Quick DOM checks
    const checks = await evalJson(session, 'JSON.stringify({imgs:document.querySelectorAll("img:not([alt])").length,inputs:document.querySelectorAll("input:not([aria-label]):not([id]):not([title])").length,emptyBtns:document.querySelectorAll("button:empty").length,roles:document.querySelectorAll("[role]").length})');
    if (checks) {
        if (checks.imgs > 0)
            emit(lines, '[A11Y]', `imgs without alt: ${checks.imgs}`);
        if (checks.inputs > 0)
            emit(lines, '[A11Y]', `inputs without label: ${checks.inputs}`);
        if (checks.emptyBtns > 0)
            emit(lines, '[A11Y]', `empty buttons: ${checks.emptyBtns}`);
...
// ─── supply-chain ─────────────────────────────────────────────────────────────
const supplyChainRecipe = async ({ session, params, signal }) => {
    const lines = [];
    await session.send('Network.enable', {});
    await session.send('Runtime.enable', {});
    ...
    if (params.url)
        await navigateAndWait(session, params.url, params.durationMs ?? 4000, signal);
    // DOM-based SRI check
    const scriptData = await evalJson(session, 'JSON.stringify([...document.querySelectorAll("script[src]")].map(s=>({src:s.src,integrity:s.integrity||null,crossorigin:s.crossOrigin||null})))');
    const domExternal = [];
    ...
    for (const domain of externalDomains)
        emit(lines, '[SUPPLY]', `  domain: ${domain}`);
    ...
    for (const s of domExternal) {
        ...
        if (!s.integrity)
            lines.push(redactEvidence(`[FINDING] NO_SRI: ${s.src.slice(0, 100)}`));
    }
    if (externalDomains.size > 10)
        lines.push(`[FINDING] HIGH_THIRD_PARTY_COUNT: ${externalDomains.size} external domains`);
    // Check for HTTP script loads
    for (const s of domExternal) {
        if (s.src.startsWith('http:'))
            emit(lines, '[FINDING]', `INSECURE_SCRIPT_LOAD: ${s.src.slice(0, 100)}`);
    }
...
// ─── inject ───────────────────────────────────────────────────────────────────
const injectRecipe = async ({ session, params }) => {
    const lines = [];
    await session.send('Page.enable', {});
    if (params.bypassCSP) {
    ...
    if (params.url) {
        ...
        if (source.includes('navigator.webdriver')) {
            const wd = await evalJson(session, 'navigator.webdriver');
...
// ─── scrape ───────────────────────────────────────────────────────────────────
const scrapeRecipe = async ({ session, params }) => {
    const lines = [];
    if (params.url)
        await navigateAndWait(session, params.url);
    await session.send('DOM.enable', {});
...
const consentRecipe = async ({ session, params, signal }) => {
    ...
    if (params.url)
        await navigateAndWait(session, params.url, params.durationMs ?? 6000, signal);
    // CMP detection
    const cmp = await evalJson(session, 'JSON.stringify({usercentrics:!!(window.UC_UI||window.usercentrics),onetrust:!!(window.OneTrust||window.Optanon),cookiebot:!!(window.CookieConsent||window.Cookiebot),trustArc:!!window.truste,iabTCF:typeof window.__tcfapi==="function",gtmLoaded:Array.isArray(window.dataLayer),dlEvents:(window.dataLayer||[]).length})');
    if (cmp) {
        emit(lines, '[CONSENT]', `CMP: ${JSON.stringify(cmp)}`);
        if (!Object.values(cmp).slice(0, 5).some(Boolean))
            lines.push('[FINDING] NO_CMP: no consent management platform detected');
    }
    // Consent state
    const cs = await evalJson(session, 'JSON.stringify({ucGcm:(()=>{try{return JSON.parse(localStorage.getItem("ucData")||"{}").gcm||null}catch{return null}})(),ucString:!!localStorage.getItem("ucString"),iabConsent:!!localStorage.getItem("eupubconsent-v2"),otConsent:!!localStorage.getItem("OptanonConsent")})');
    if (cs?.ucGcm && typeof cs.ucGcm === 'object') {
        const gcm = cs.ucGcm;
        emit(lines, '[CONSENT]', `GCM flags: ${JSON.stringify(gcm)}`);
        if (gcm['adStorage'] === 'granted')
            lines.push('[FINDING] CONSENT_PRE_GRANTED: adStorage=granted');
        if (gcm['analyticsStorage'] === 'granted')
            lines.push('[FINDING] CONSENT_PRE_GRANTED: analyticsStorage=granted');
    }
    if (cs?.iabConsent)
        emit(lines, '[CONSENT]', 'IAB TCF v2 consent string present');
    if (cs?.otConsent)
        emit(lines, '[CONSENT]', 'OneTrust consent string present');
    // Tracker pre-grant
    emit(lines, '[CONSENT]', `trackers on cold load: ${trackerHits.length}`);
    for (const h of trackerHits.slice(0, 10))
        emit(lines, '[CONSENT]', `  tracker: ${h}`);
    if (trackerHits.length > 0)
        lines.push(`[FINDING] TRACKERS_BEFORE_CONSENT: ${trackerHits.length} tracker requests fired before consent`);
    // dataLayer events
    const dl = await evalJson(session, 'JSON.stringify((window.dataLayer||[]).slice(0,10).map(e=>e.event||JSON.stringify(e).slice(0,60)))');
    if (dl?.length)
        emit(lines, '[CONSENT]', `dataLayer events: ${dl.join(', ')}`);
...
const emulateRecipe = async ({ session, params, screenshotDir }) => {
    const lines = [];
    await session.send('Page.enable', {});
    await session.send('Network.enable', {});
    const d = params.device;
    const presetKey = d?.preset ?? '';
    ...
    const throttleKey = params.throttlePreset;
    ...
    if (params.url) {
        ...
        const v = await evalJson(session, 'JSON.stringify({width:window.innerWidth,dpr:devicePixelRatio,mobile:/Mobi/.test(navigator.userAgent)})');
        if (v)
            emit(lines, '[EMULATE]', `verified: innerWidth=${v.width} dpr=${v.dpr} mobileUA=${v.mobile}`);
...
// ─── monitor ──────────────────────────────────────────────────────────────────
const monitorRecipe = async ({ session, params, signal }) => {
    const lines = [];
    ...
    session.on('Network.responseReceived', (ev) => {
        ...
        if (status >= 400)
            queue.push(`HTTP ${status} ${e.response?.url?.slice(0, 80)}`);
    });
    if (params.url)
        await navigateAndWait(session, params.url, Math.min(4000, durationMs / 2), signal);
    emit(lines, '[MONITOR]', `watching for ${durationMs}ms at ${intervalMs}ms intervals`);
    const endTime = Date.now() + durationMs;
    let iteration = 0;
    while (Date.now() < endTime && !signal?.aborted) {
        const snapshot = await evalJson(session, 'JSON.stringify({url:location.href,title:document.title,errorEls:document.querySelectorAll(".error,[aria-invalid=\"true\"],[data-error]").length})');
        if (snapshot) {
            ...
            if (prevUrl && prevUrl !== snapshot.url)
                emit(lines, '[FINDING]', `MONITOR_REDIRECT: URL changed to ${snapshot.url}`);
            if (snapshot.errorEls > 0)
                lines.push(`[FINDING] MONITOR_DOM_ERROR: ${snapshot.errorEls} error elements at t=${iteration * intervalMs / 1000}s`);
            prevUrl = snapshot.url;
        }
        while (queue.length)
            lines.push(`[FINDING] MONITOR_EVENT: ${redactEvidence(queue.shift())}`);
        iteration++;
...
// ─── workers ──────────────────────────────────────────────────────────────────
const workersRecipe = async ({ session, params, signal }) => {
    const lines = [];
    ...
    session.on('Target.attachedToTarget', async (ev) => {
        const e = ev;
        const type = e.targetInfo?.type ?? '';
        const url = e.targetInfo?.url ?? '';
        const sessionId = e.sessionId ?? '';
        if (!['worker', 'shared_worker', 'service_worker'].includes(type))
            return;
        workerSessions.push({ type, url, sessionId });
        emit(lines, '[WORKER]', `attached: type=${type} url=${url || '(blob)'}`);
        if (type === 'worker' && !url)
            lines.push('[FINDING] WORKER_BLOB: blob worker (opaque)');
        if (type === 'shared_worker')
            lines.push(`[FINDING] SHARED_WORKER: ${url}`);
        if (type === 'service_worker')
            lines.push(`[FINDING] SERVICE_WORKER_TARGET: ${url}`);
    });
    session.on('ServiceWorker.workerVersionUpdated', (ev) => {
        const e = ev;
        for (const v of e.versions ?? []) {
            emit(lines, '[SW]', `version: status=${v.status} running=${v.runningStatus} script=${v.scriptURL}`);
            if (v.status === 'activated')
                emit(lines, '[FINDING]', `SW_ACTIVATED: ${v.scriptURL}`);
        }
    });
    session.on('ServiceWorker.workerRegistrationUpdated', (ev) => {
        const e = ev;
        for (const r of e.registrations ?? []) {
            emit(lines, '[SW]', `registered: scope=${r.scopeURL} deleted=${r.isDeleted}`);
            if (!r.isDeleted)
                lines.push(`[FINDING] SW_REGISTERED: ${r.scopeURL}`);
        }
    });
    // Existing targets
    ...
    for (const t of existing)
        emit(lines, '[WORKER]', `  ${t.type}: ${t.url || '(blob)'}`);
    if (params.url) {
        await session.send('Page.enable', {});
    ...
    // SW snapshot via JS
    const swSnap = await evalJson(session, 'navigator.serviceWorker?navigator.serviceWorker.getRegistrations().then(r=>JSON.stringify(r.map(s=>({scope:s.scope,state:(s.active||s.installing||s.waiting)?.state||"none",script:(s.active||s.installing||s.waiting)?.scriptURL})))):JSON.stringify([])');
    if (swSnap?.length) {
        emit(lines, '[SW]', `runtime snapshot: ${swSnap.length} registration(s)`);
        for (const sw of swSnap)
            emit(lines, '[SW]', `  scope=${sw.scope} state=${sw.state}`);
    }
...
// ─── service-worker ───────────────────────────────────────────────────────────
const serviceWorkerRecipe = async ({ session, params, signal }) => {
    const lines = [];
    const durationMs = params.durationMs ?? 6000;
    await session.send('ServiceWorker.enable', {});
    session.on('ServiceWorker.workerRegistrationUpdated', (ev) => {
        ...
        for (const r of e.registrations ?? []) {
            if (!r.isDeleted) {
                emit(lines, '[SW]', `registered: scope=${r.scopeURL}`);
                lines.push(`[FINDING] SW_REGISTERED: ${r.scopeURL}`);
            }
            else
                lines.push(`[FINDING] SW_REMOVED: ${r.scopeURL}`);
        }
    });
    session.on('ServiceWorker.workerVersionUpdated', (ev) => {
        const e = ev;
        for (const v of e.versions ?? []) {
            ...
            if (v.status === 'activated')
                emit(lines, '[FINDING]', `SW_ACTIVATED: ${v.scriptURL}`);
            const host = (() => { try {
                return new URL(v.scriptURL ?? '').hostname;
            }
            catch {
                return '';
            } })();
            const pageHost = (() => { try {
                return new URL(params.url ?? 'about:blank').hostname;
            }
            catch {
                return '';
            } })();
            if (host && pageHost && host !== pageHost)
                emit(lines, '[FINDING]', `SW_THIRD_PARTY_SCRIPT: ${v.scriptURL}`);
        }
    });
    if (params.url)
        await navigateAndWait(session, params.url, Math.min(4000, durationMs), signal);
    ...
    const regs = await evalJson(session, 'navigator.serviceWorker?navigator.serviceWorker.getRegistrations().then(r=>JSON.stringify(r.map(x=>({scope:x.scope,state:(x.active||x.installing||x.waiting)?.state||"none",script:(x.active||x.installing||x.waiting)?.scriptURL})))):JSON.stringify([])');
    if (regs !== null) {
        emit(lines, '[SW]', `snapshot: ${regs.length} registration(s)`);
        for (const r of regs ?? [])
            emit(lines, '[SW]', `  scope=${r.scope} state=${r.state} script=${r.script}`);
    }
    await session.send('ServiceWorker.disable', {}).catch(() => { });
    return { evidenceLines: lines, details: { registrations: regs?.length ?? 0 } };
};
// ─── websocket ────────────────────────────────────────────────────────────────
const websocketRecipe = async ({ session, params, signal }) => {
    const lines = [];
    const durationMs = params.durationMs ?? 10_000;
    await session.send('Network.enable', {});
    const wsMap = new Map();
    session.on('Network.webSocketCreated', (ev) => {
        const e = ev;
        if (e.requestId && e.url) {
            wsMap.set(e.requestId, { url: e.url, frames: 0 });
            emit(lines, '[WS]', `created: ${e.url}`);
        }
        if (e.url) {
            const host = (() => { try {
            ...
            if (params.url && (() => { try {
                return new URL(params.url).hostname !== host;
            }
            catch {
                return false;
            } })())
    ...
    for (const ev of ['Network.webSocketFrameSent', 'Network.webSocketFrameReceived']) {
        session.on(ev, (e) => {
            ...
            if (/token|password|key|secret/i.test(payload))
                lines.push('[FINDING] SENSITIVE_IN_WS_FRAME: sensitive data detected');
            if (payload.length > 102400)
                lines.push(`[FINDING] LARGE_WS_FRAME: ${Math.round(payload.length / 1024)}KB`);
        });
    }
    session.on('Network.webSocketClosed', (ev) => {
        const e = ev;
        const ws = wsMap.get(e.requestId ?? '');
        if (ws)
            emit(lines, '[WS]', `closed: ${ws.url} frames=${ws.frames}`);
    });
    if (params.url)
        await navigateAndWait(session, params.url, Math.min(4000, durationMs), signal);
    await new Promise(r => setTimeout(r, Math.min(Math.max(durationMs - 4000, 0), 2000)));
    ...
    for (const [, ws] of wsMap)
        emit(lines, '[WS]', `  ${ws.url} total_frames=${ws.frames}`);
    return { evidenceLines: lines, details: { wsCount: wsMap.size } };
};
// ─── memory ───────────────────────────────────────────────────────────────────
const memoryRecipe = async ({ session, params }) => {
    const lines = [];
    if (params.url)
        await navigateAndWait(session, params.url);
    await session.send('Performance.enable', {});
    ...
    if ((dom.nodes ?? 0) > 10_000)
        lines.push(`[FINDING] LARGE_DOM: ${dom.nodes} nodes`);
    if ((dom.jsEventListeners ?? 0) > 5_000)
        lines.push(`[FINDING] HIGH_LISTENER_COUNT: ${dom.jsEventListeners}`);
    const perf = await session.send('Performance.getMetrics', {});
    const m = (perf.metrics ?? []).reduce((a, { name, value }) => (a[name] = value, a), {});
    ...
    if (heapTotal > 0 && heapUsed / heapTotal > 0.9)
        lines.push(`[FINDING] HEAP_PRESSURE: ${pct}% heap used`);
    return { evidenceLines: lines, details: { domNodes: dom.nodes ?? 0, heapMB: Math.round(heapUsed / 1024 / 1024) } };
};
// ─── css-coverage ─────────────────────────────────────────────────────────────
const cssCoverageRecipe = async ({ session, params }) => {
    const lines = [];
    await session.send('DOM.enable', {});
    await session.send('CSS.enable', {});
    await session.send('CSS.startRuleUsageTracking', {});
    if (params.url)
        await navigateAndWait(session, params.url, params.durationMs ?? 4000);
    const result = await session.send('CSS.stopRuleUsageTracking', {});
    ...
    const bySheet = rules.reduce((a, r) => {
        if (!a[r.styleSheetId])
            a[r.styleSheetId] = { used: 0, total: 0 };
        a[r.styleSheetId].total++;
        if (r.used)
            a[r.styleSheetId].used++;
        return a;
    }, {});
...
// ─── js-coverage ──────────────────────────────────────────────────────────────
const jsCoverageRecipe = async ({ session, params }) => {
    const lines = [];
    ...
    for (const script of scripts) {
        ...
        coveredBytes += c;
        if (t > 0)
            byScript.push({ url: script.url, pct: (c / t) * 100, unused: t - c });
    }
...
// ─── login ────────────────────────────────────────────────────────────────────
const loginRecipe = async ({ session, params, signal }) => {
    const lines = [];
    ...
    emit(lines, '[AUTH]', `current URL: ${((await session.send('Runtime.evaluate', { expression: 'location.href', returnByValue: true })).result?.value) ?? '?'}`);
    ...
    while (Date.now() < endTime && !signal?.aborted) {
        await new Promise(r => setTimeout(r, 2000));
        const currentUrl = (await session.send('Runtime.evaluate', { expression: 'location.href', returnByValue: true }).catch(() => ({ result: { value: '' } }))).result?.value ?? '';
...
// ─── full-audit ───────────────────────────────────────────────────────────────
const fullAuditRecipe = async ({ session, params, screenshotDir, signal }) => {
    const lines = [];
    ...
    const runScheme = async (label, recipe) => {
        try {
            ...
            emit(lines, '[AUDIT]', `${label} error: ${err.message.slice(0, 80)}`);
        }
    };

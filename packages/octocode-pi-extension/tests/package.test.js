...
import { MANAGED_BLOCK_END, MANAGED_BLOCK_START, OCTOCODE_DIRECT_TOOL_NAMES, OCTOCODE_SUPPORT_TOOL_NAMES, createAwarenessBridge, disableBuiltinReadTool, extractWriteTargetPaths, formatStatus, applyOctocodeUi, getThinkingStatus, getAssetPaths, getCLIPath, getAppendSystemTarget, getInstallSource, listBundledSkills, listExtensionHarness, mergeManagedAppendSystem, parseSetupScope, readTextIfExists, shouldAppendSystemPrompt, splitArgs, truncateUserVisibleToolOutput, cleanupSpawnedAgentsForShutdown, setAgentProcessFactoryForTests, } from '../src/index.js';
...
function ensureDistAssetsForUnitTests() {
    if (distAssetsReady)
        return;
    ...
    return async () => {
        ...
        try {
            ...
            if (previous === undefined)
                delete process.env['OCTOCODE_MEMORY_HOME'];
            else
                process.env['OCTOCODE_MEMORY_HOME'] = previous;
...
function assertHasAllOctocodeSkills(skillArgs) {
    ...
    assert.ok(bundled.includes('octocode-awareness'), 'octocode-awareness is always bundled');
    const expected = EXPECTED_OCTOCODE_SKILLS.filter(name => bundled.includes(name));
    for (const skillName of expected) {
        assert.ok(skillArgs.some(skillPath => skillPath.endsWith(path.join('skills', skillName))), `missing bundled skill: ${skillName}`);
    }
}
...
test('build composes the system prompt from its section files', async () => {
    ...
    for (const file of sourceFiles) {
        assert.equal(fs.readFileSync(path.join(distSections, file), 'utf8'), fs.readFileSync(path.join(sourceSections, file), 'utf8'), `dist prompt section differs from source: ${file}`);
    }
    for (const agent of ['architect', 'browser-agent', 'planner', 'researcher']) {
        assert.equal(fs.readFileSync(path.join(distDir, 'subagents', agent, 'SYSTEM_PROMPT.md'), 'utf8'), fs.readFileSync(path.join(packageRoot, 'subagents', agent, 'SYSTEM_PROMPT.md'), 'utf8'), `dist subagent prompt differs from source: ${agent}`);
    }
});
test('build copies bundled Octocode skills without secret env files', () => {
    assert.equal(path.basename(getCLIPath(distDir)), 'octocode.js', 'Octocode CLI resolves to an octocode.js entry (dist bundle or node_modules fallback)');
    ...
    assert.deepEqual(sourceSkills, rootSkills, 'every repo-root skill is staged into the npm package');
    for (const skill of rootSkills) {
        assert.equal(fs.readFileSync(path.join(packageRoot, 'skills', skill, 'SKILL.md'), 'utf8'), fs.readFileSync(path.resolve(packageRoot, '../..', 'skills', skill, 'SKILL.md'), 'utf8'), `${skill} SKILL.md is copied from the repo-root bundle`);
    }
    assert.ok(skills.includes('octocode-awareness'), 'canonical octocode-awareness skill is always bundled');
    assert.equal(fs.readFileSync(path.join(distDir, 'skills', 'octocode-awareness', 'SKILL.md'), 'utf8'), fs.readFileSync(path.resolve(packageRoot, '../octocode-awareness/out/skills/octocode-awareness/SKILL.md'), 'utf8'));
    ...
    assert.match(fs.readFileSync(path.resolve(packageRoot, '../..', '.gitignore'), 'utf8'), /^\/packages\/octocode-pi-extension\/skills\/$/m, 'generated npm skill staging remains gitignored');
...
test('path, asset, and output helpers cover edge cases', () => {
    assert.equal(getAppendSystemTarget('global', '/repo', '/home/tester'), path.join('/home/tester', '.pi', 'agent', 'APPEND_SYSTEM.md'));
    ...
    try {
        ...
        assert.throws(() => assertPathAllowed(path.join(path.parse(packageRoot).root, 'octocode-pi-blocked-outside-root', 'x.txt'), packageRoot, 'test write'), /outside the allowed roots/);
    }
    finally {
        if (previousAllowed === undefined)
            delete process.env['ALLOWED_PATHS'];
        else
            process.env['ALLOWED_PATHS'] = previousAllowed;
    }
});
...
test('formatStatus reports the dist assets and Awareness runtime', withTempMemoryHome(() => {
    const status = formatStatus(distDir);
    assert.match(status, /system prompt: found/);
    assert.match(status, /octocode-awareness/);
    assert.match(status, /awareness runtime: @octocodeai\/octocode-awareness \(bundled runtime\)/);
    ...
    assert.match(status, /disabled\/replaced built-ins: overridden: edit, write, bash/);
...
test('write target extraction supports Pi write and edit inputs', () => {
    ...
    assert.deepEqual(extractWriteTargetPaths('write', {
        path: ' src/a.js ',
        filePaths: ['src/b.js', 'src/a.js'],
    }), ['src/a.js', 'src/b.js']);
    assert.deepEqual(extractWriteTargetPaths('edit', {
        file_path: 'src/c.js',
        paths: ['src/d.js'],
    }), ['src/c.js', 'src/d.js']);
});
test('awareness bridge aggregates advisory work and finalizes its automatic HOOK run PENDING at shutdown', withIsolatedDb(async (ctx) => {
    await withAgentId('pi-test-agent', async () => {
        ...
        const locks = db.prepare('SELECT COUNT(*) AS c FROM locks').get();
        assert.equal(locks.c, 0, 'ordinary bridge edits do not acquire exclusive locks');
        const activePresence = db.prepare('SELECT COUNT(*) AS c FROM run_files WHERE ended_at IS NULL').get();
        ...
        assert.equal(beforeShutdown.prepare("SELECT COUNT(*) AS c FROM task_runs WHERE status='ACTIVE'").get().c, 1);
        ...
        const pending = db2
            ...
            .get();
        assert.equal(pending.c, 1, 'shutdown finalizes the aggregate PENDING (verification still owed)');
        const noLocks = db2.prepare('SELECT COUNT(*) AS c FROM locks').get();
        assert.equal(noLocks.c, 0, 'advisory work never created a lock row');
        const endedPresence = db2.prepare('SELECT COUNT(*) AS c FROM run_files WHERE ended_at IS NOT NULL').get();
...
test('replaces built-in write by custom tool override with path guard', async () => {
    ...
    try {
        ...
        await assert.rejects(() => invokeExecute(writeTool, { path: outside, content: 'x' }, { cwd: tmp }), /write blocked|outside the allowed roots/);
    }
    finally {
        fs.rmSync(tmp, { recursive: true, force: true });
    }
...
test('write records read-state so a follow-up edit is not stale', async () => {
    ...
    try {
        ...
        await invokeExecute(writeTool, { path: target, content: 'const x = 1;\n' }, { cwd: tmp });
        const edited = await invokeExecute(editTool, {
            path: target,
...
test('custom edit applies batched replacements and replaceAll against original content', () => {
    const result = applyCustomEditsToContent('alpha one\nbeta one\nalpha two\n', [
        {
            oldText: 'beta one',
            newText: 'beta two',
...
test('custom edit not-found errors include current-file recovery guidance', () => {
    assert.throws(() => applyCustomEditsToContent('const value = 1;\n', [
        {
            oldText: 'const value = 2;',
            newText: 'const value = 3;',
            reasoning: 'test',
        },
    ], 'sample.ts'), /Re-read the target range and retry with a smaller unique oldText/);
});
test('custom edit not-found diagnostics preserve visible leading whitespace in similar-line hints', () => {
    ...
    ], 'sample.ts'), /line 1: ····const value = 1;/);
});
test('custom edit requires reasoning and shows it in output', async () => {
    ...
    try {
        ...
        await assert.rejects(() => invokeExecute(editTool, {
            path: target,
...
test('custom edit renderResult lists per-edit reasoning, red/green diff, line range, and file', async () => {
    ...
    try {
        ...
        assert.ok(themedLines.some(l => /rename to v2 handler/.test(l) &&
            !l.includes('- ') &&
            !l.includes('+ ')), 'edit #1 reasoning shown');
        assert.ok(themedLines.some(l => /rename total to sumTotal for clarity/.test(l) &&
            !l.includes('- ') &&
            !l.includes('+ ')), 'edit #2 reasoning shown');
...
test('BREAK: BOM + CRLF file round-trips through an edit preserving BOM and CRLF', async () => {
    ...
    try {
        ...
        assert.ok(result.details.files[0].edits
            .length > 0, 'per-edit evidence present');
    }
    finally {
        clearEditReadStateForTests();
        fs.rmSync(tmp, { recursive: true, force: true });
...
test('BREAK: lineRange with matching oldText succeeds; mismatched oldText throws', () => {
    ...
    assert.throws(() => applyCustomEditsToContent('one\ntwo\nthree\n', [
        {
            newText: 'TWO\n',
            matchMode: 'lineRange',
            startLine: 2,
    ...
    ], 'sample.txt'), /oldText does not match the requested line range/);
});
test('BREAK: multi-file edit is all-or-nothing when one query requires read state it lacks', async () => {
    ...
    try {
        await assert.rejects(() => invokeExecute(tools.get('edit'), {
            queries: [
                {
                    path: a,
                    requireRecentRead: true,
...
test('BREAK: an already-aborted signal rejects before any file read', async () => {
    ...
    try {
        ...
        }, ctrl.signal, undefined, { cwd: process.cwd() }), /Operation aborted/);
...
test('custom edit supports normalized and lineRange match modes', () => {
    const normalized = applyCustomEditsToContent('const label = “hello”;\n', [
        {
    ...
    const lineRange = applyCustomEditsToContent('one\ntwo\nthree\n', [
        {
            newText: 'TWO\n',
            matchMode: 'lineRange',
            startLine: 2,
...
test('custom edit rejects a non-unique oldText without replaceAll', () => {
    assert.throws(() => applyCustomEditsToContent('dup\ndup\n', [{ oldText: 'dup', newText: 'DUP', reasoning: 'test' }], 'sample.txt'), /Found 2 occurrences/);
});
test('custom edit lineRange rejects an out-of-range range', () => {
    ...
    ], 'sample.txt'), /line range 1-99 is outside/);
});
...
test('registers all Octocode direct tools as native Pi tools', async () => {
    const { tools } = await captureExtensions();
    assert.deepEqual(OCTOCODE_DIRECT_TOOL_NAMES.filter(toolName => !tools.has(toolName)), [], 'every direct Octocode tool is registered as a Pi tool');
    assert.equal(OCTOCODE_DIRECT_TOOL_NAMES.length, 13);
    assert.ok(!OCTOCODE_DIRECT_TOOL_NAMES.includes('unzip'), 'unzip is not a native Pi tool — use npx octocode unzip via bash');
    for (const toolName of OCTOCODE_DIRECT_TOOL_NAMES) {
        ...
        assert.equal(tool.parameters['type'], 'object', `${toolName} exposes an object schema`);
        assert.ok(tool.parameters.properties, `${toolName} exposes schema properties`);
    ...
    const props = localViewStructure.parameters.properties;
    assert.ok(props['queries'], 'bulk CLI tool schema exposed to Pi');
    const queriesItems = props['queries'].items;
    ...
    assert.deepEqual(localViewStructure.renderCall({ queries: [{ path: packageRoot }] }, theme)
        .render(80)[0]
        .includes('localViewStructure'), true);
    assert.deepEqual(localViewStructure.renderResult({
        isError: false,
    ...
    assert.ok(previewVisibleWidth <= 80, `expanded preview line visible width (${previewVisibleWidth}) must not exceed render width 80`);
...
test('applies Octocode Pi UI status and hidden thinking label', () => {
    ...
    applyOctocodeUi({
        ...
        ui: {
            ...
            setHiddenThinkingLabel: (label) => calls.push(['thinking', label]),
    ...
    assert.equal(getThinkingStatus({ model: { id: 'gpt-5.5', reasoning: false } }, 'high'), 'thinking: off (gpt-5.5 has reasoning:false)');
    assert.equal(getThinkingStatus({ model: { id: 'claude', reasoning: true } }, 'high'), 'thinking: high (claude)');
});
...
test('disableBuiltinReadTool is defensive and only removes disabled built-ins', () => {
    assert.equal(disableBuiltinReadTool({}), false);
    ...
    assert.throws(() => disableBuiltinReadTool({
        getActiveTools: () => {
...
test('extension commands and lifecycle handlers execute user-visible wiring paths', async () => {
    ...
    const ctx = {
        ...
        ui: {
            ...
            setHiddenThinkingLabel: (label) => statuses.push(['hidden-thinking', label]),
        },
    };
    try {
...
test('manage_context type:compact queues a continuation after compaction completes', async () => {
    ...
    let compactOptions = {};
    const notifications = [];
    const result = await invokeExecute(compactTool, { type: 'compact', instructions: 'focus on recent file changes' }, {
...
test('manage_context type:compact reports compaction errors without queueing continuation', async () => {
    ...
    let compactOptions = {};
    const notifications = [];
    await invokeExecute(compactTool, { type: 'compact' }, {
...
test('manage_context type:new, missing compact support, and render states are explicit', async () => {
    ...
    await assert.rejects(() => invokeExecute(compactTool, { type: 'compact' }, {}), /ctx\.compact is not available/);
    assert.match(compactTool.renderCall({ type: 'new' }, {
        bold: (text) => `<b>${text}</b>`,
        fg: (_color, text) => text,
    }).render(120)[0], /manage_context<\/b> \(new\)/);
    assert.equal(compactTool.renderResult({ isError: false, content: [{ type: 'text', text: 'ok' }] }, { isPartial: true }).render(120)[0], 'Processing…');
    assert.equal(compactTool.renderResult({ isError: true, content: [{ type: 'text', text: 'bad' }] }, { expanded: false }).render(120)[0], '✗ manage_context');
    const { commands } = await captureExtensions();
    const notifications = [];
    ...
    await commands.get('_octocode-clear-context-impl').handler('', {
        ...
        ui: {
            notify: (message, level) => cancelled.push({ message, level }),
        },
    });
...
test('turn_end auto-compact queues a continuation after compaction completes (no stuck agent)', async () => {
    ...
    const onComplete = compactOptions.onComplete;
...
test('turn_end auto-compact reports errors without queueing a continuation', async () => {
    ...
    const onError = compactOptions.onError;
...
test('lists every extension harness surface', () => {
    ...
    assert.match(harness.awarenessCliNote, /bundled Awareness CLI.*awareness\.mjs/, 'awarenessCliNote shows bundled Awareness CLI path');
...
test('native Octocode local tool executes through the Pi tool wrapper', async () => {
    ...
    try {
        ...
        const result = await invokeExecute(tools.get('localViewStructure'), { queries: [{ path: tmp, filesOnly: true }] }, { cwd: packageRoot });
...
test('native Octocode tool wrapper preserves responseCharLength pagination in text output', async () => {
    ...
    try {
        ...
        const result = await invokeExecute(tools.get('localViewStructure'), { queries: [{ path: tmp, filesOnly: true }], responseCharLength: 80 }, { cwd: packageRoot });
        ...
        assert.equal(result.details
            ?.responsePagination?.hasMore, true);
    }
    finally {
        fs.rmSync(tmp, { recursive: true, force: true });
    }
...
test('native Octocode tool wrapper throws so Pi marks execution failed', async () => {
    const { tools } = await captureExtensions();
    await assert.rejects(() => invokeExecute(tools.get('localViewStructure'), { queries: [{}] }, { cwd: packageRoot }), /path|expected string/);
});
test('awareness bridge fails open on non-conflict errors', async () => {
    await withAgentId('pi-test-agent', async () => {
        const messages = [];
        const bridge = createAwarenessBridge();
        const result = await bridge.handleToolCall({ toolName: 'write', toolCallId: 'tool-3', input: { path: 'src/a.js' } }, {
            cwd: '/repo',
            dbPath: '/dev/null/cannot-create-dir/awareness.sqlite3',
            ui: {
                notify: (message, level) => messages.push({ level: level ?? 'info', message }),
            },
        });
...
test('spawnAgent starts a lean RPC Pi process and AgentMessage can list/status/send', async () => {
    ...
    try {
        ...
        assert.match(String(spawnTool.parameters.properties.model?.description ?? ''), /pi -ne --list-models/);
        ...
        const result = await invokeExecute(spawnTool, {
            task: 'check the docs',
        ...
        const agentId = result.details.agent
            .agentId;
...
test('spawnAgent gives each worker a distinct Awareness identity', async () => {
    ...
    try {
        await withAgentId('parent-agent', async () => {
            ...
            const workerIds = spawned.map(item => item.options.env?.['OCTOCODE_AGENT_ID']);
            assert.equal(spawned.length, 2);
            assert.ok(workerIds.every(id => id?.startsWith('parent-agent:worker:')), 'worker identities preserve the parent prefix');
            assert.notEqual(workerIds[0], 'parent-agent');
            assert.notEqual(workerIds[0], workerIds[1]);
            assert.ok(spawned.every(item => item.options.env?.['OCTOCODE_PI_SUBAGENT'] === '1'));
...
test('spawnAgent covers octocode resource options, prompt file cleanup, list renderers, and dead-worker messaging', async () => {
    ...
    try {
        ...
        assert.ok(messageTool.renderResult(list, { expanded: true }, theme).render(160)
            .length > 1);
        assert.equal(messageTool.renderResult(list, { isPartial: true }, theme).render(120)[0], '<warning>⧗ Agent working…</warning>');
        const noOutputStatus = await invokeExecute(messageTool, {
            action: 'status',
            agentId,
        });
        const noOutputCollapsed = messageTool.renderResult(noOutputStatus, { expanded: false }, theme).render(240)[0];
        ...
        const outputCollapsed = messageTool.renderResult(outputStatus, { expanded: false }, theme).render(240)[0];
        ...
        await assert.rejects(() => invokeExecute(messageTool, {
            action: 'followUp',
            agentId,
            message: 'too late',
        }), /cannot reach agent/);
        ...
        }, { expanded: true }, theme).render(120);
...
test('spawnSubagent starts the browser-agent with the typed prompt, tools, all Octocode skills, and octocode resource mode', async () => {
    ...
    try {
        ...
        const result = await invokeExecute(spawnSubagent, {
            agent: 'browser-agent',
        ...
        assert.ok(skillArgs.some(skillPath => skillPath.endsWith(path.join('subagents', 'browser-agent', 'skills', 'browser-agent'))), 'browser subagent should load its browser-agent skill');
        assert.ok(args.includes('--tools'));
...
test('spawnSubagent starts researcher, planner, and architect with all Octocode skills', async () => {
    ...
    try {
        const { tools } = await captureExtensions();
        const spawnSubagent = tools.get('spawnSubagent');
        const schema = spawnSubagent.parameters;
        ...
        assert.match(spawnSubagent.promptGuidelines.join('\n'), /hardcoded config paths/);
        assert.match(String(spawnSubagent.parameters.properties.model?.description ?? ''), /pi -ne --list-models/);
        ...
        for (const args of [researcherArgs, plannerArgs, architectArgs]) {
            ...
            assert.equal(argValues(args, '--skill').some(skillPath => skillPath.includes(`${path.sep}subagents${path.sep}browser-agent${path.sep}`)), false, 'non-browser specialists should not load the browser-agent skill');
...
test('spawnSubagent covers context injection, invalid URL name fallback, unknown agent, and render fallback', async () => {
    ...
    try {
        ...
        const initialPrompt = JSON.parse(spawned[0].proc.stdinWrites[0]);
        assert.match(initialPrompt.message, /## Context\nPrior finding/);
        assert.match(result.content[0].text, /\[SPAWNED\] Browser Agent · agentId:/);
        assert.match(result.content[0].text, /\[SPAWNED\] name: Browser Agent · /);
        await assert.rejects(() => invokeExecute(spawnSubagent, { agent: 'missing-agent', task: 'nope' }), /Unknown subagent/);
        assert.match(spawnSubagent.renderCall({
            agent: 'missing-agent',
            task: 'x'.repeat(80),
        }).render(120)[0], /spawnSubagent\(missing-agent\).*…/);
    }
    finally {
        setAgentProcessFactoryForTests(null);
    }
...
test('spawnSubagent remains available for non-browser specialists when Chrome debug is disabled', async () => {
    ...
    try {
        ...
        await assert.rejects(() => invokeExecute(spawnSubagent, {
            agent: 'browser-agent',
        ...
        if (previous === undefined)
            delete process.env['OCTOCODE_CHROME_DEBUG'];
        else
            process.env['OCTOCODE_CHROME_DEBUG'] = previous;
    }
});
test('spawnAgent does not register recursively inside spawned workers', async () => {
    ...
    try {
        ...
        if (previous === undefined)
            ...
            process.env['OCTOCODE_PI_SUBAGENT'] = previous;
    }
});
test('AgentMessage wait collects worker output and kill terminates stale workers', async () => {
    ...
    try {
        ...
        const firstId = first.details.agent
            .agentId;
        ...
        assert.equal(runningStatus.details.agent
            .activeTool, 'localSearchCode');
        spawned[0].emitStdout({
            type: 'tool_result',
            toolCallId: 'tool-1',
            toolName: 'localSearchCode',
        ...
        assert.equal(waited.details.agent.status, 'idle');
        ...
        const secondId = second.details.agent
            .agentId;
...
test('AgentMessage abort sends Pi RPC abort command without killing the process', async () => {
    ...
    try {
        ...
        const actionSchema = messageTool.parameters.properties?.action;
        assert.ok(Array.isArray(actionSchema?.enum) && actionSchema.enum.includes('abort'), 'abort must be in AgentMessage action schema');
...
test('evictStaleAgents removes oldest terminal agents when registry reaches MAX_AGENT_RECORDS', async () => {
    ...
    try {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            ids.push(r.details.agent.agentId);
        }
        ...
        const overflowId = overflow.details
            .agent.agentId;
        ...
        const agentCount = list.details.agents.length;
        assert.ok(agentCount <= 50, `Registry must stay ≤ 50 agents, got ${agentCount}`);
        // ids[0] (oldest terminal) must be gone
        await assert.rejects(() => invokeExecute(messageTool, { action: 'status', agentId: ids[0] }), /No agent found/, 'Oldest evicted agent must not be in the registry');
        void overflowId;
    }
    finally {
        setAgentProcessFactoryForTests(null);
...
test('waitForAgent timeout error uses agent name, not internal UUID', async () => {
    ...
    try {
        ...
        await assert.rejects(() => invokeExecute(messageTool, { action: 'wait', agentId, timeoutMs: 1 }), (err) => {
...
test('getAgent throws actionable error for missing or unknown agentId', async () => {
    ...
    try {
        ...
        await assert.rejects(() => invokeExecute(messageTool, { action: 'status' }), (err) => {
...
test('AgentMessage wait with remove:true cleans up agent from registry after completion', async () => {
    ...
    try {
        ...
        await assert.rejects(() => invokeExecute(messageTool, { action: 'status', agentId }), /No agent found/, 'Agent must be removed from registry after wait+remove');
    }
    finally {
        setAgentProcessFactoryForTests(null);
    }
...
test('RPC response with success:false surfaces error in agent result', async () => {
    ...
    try {
        ...
        assert.match(status.content[0].text, /already streaming|streamingBehavior|RPC command failed/, 'RPC error must appear in agent status output');
...
test('AgentMessage action schema includes all documented actions', async () => {
    ...
    for (const action of expectedActions) {
        assert.ok(actionSchema?.enum?.includes(action), `AgentMessage action schema must include "${action}"`);
    }
});

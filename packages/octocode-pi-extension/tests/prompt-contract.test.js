...
const browserSkillDir = path.join(packageRoot, 'subagents', 'browser-agent', 'skills', 'browser-agent');
function activeSkillCatalog() {
    const content = fs.readFileSync(path.join(packageRoot, 'src', 'prompts', 'sections', 'skills.md'), 'utf8');
    return [...content.matchAll(/^- `([a-z0-9-]+)` —/gm)].map(match => match[1]);
}
...
test('every active skill catalog entry resolves to a shipped SKILL.md or carries an explicit install command', () => {
    ...
    for (const skill of skills) {
        ...
        const catalogLine = content.match(new RegExp(`^- \`${skill}\` —.*$`, 'm'))?.[0] ?? '';
        const documentedInstall = /install once|npx octocode skill/i.test(catalogLine);
        assert.ok(candidates.some(candidate => fs.existsSync(candidate)) || documentedInstall, `prompt references missing skill: ${skill}`);
    }
});
...
test('octocode-cli: every skill --name command carries --platform', () => {
    ...
    for (const line of skillNameLines) {
        ...
        assert.ok(isNpx || hasPlatform, `skill --name line is missing --platform (use --platform pi or npx form): ${line.trim()}`);
    }
});
test('work-mode: LEARN/CLEAN/PROJECT phases carry inline annotations', () => {
    ...
    assert.match(SYSTEM_PROMPT, /LEARN\?\s*\(/, 'LEARN? phase must have an inline annotation e.g. LEARN? (…)');
    assert.match(SYSTEM_PROMPT, /CLEAN\?\s*\(/, 'CLEAN? phase must have an inline annotation e.g. CLEAN? (…)');
    assert.match(SYSTEM_PROMPT, /PROJECT\?\s*\(/, 'PROJECT? phase must have an inline annotation e.g. PROJECT? (…)');
});
test('agents: -ne flag is explained inline', () => {
    // Item 5: `pi -ne --list-models` was shown without explaining what -ne means.
    assert.match(SYSTEM_PROMPT, /-ne[^\n]*non-interactive|non-interactive[^\n]*-ne/i, '`-ne` must be explained as non-interactive (and/or no-extensions) near its usage');
});
test('awareness: no stale hardcoded model names', () => {
    // Item 6: "Haiku or Composer 2.5" is stale and version-locks the prompt.
    assert.doesNotMatch(SYSTEM_PROMPT, /\bHaiku\b/, 'Haiku is a stale model name — use generic phrasing like "the configured fast/small model"');
    assert.doesNotMatch(SYSTEM_PROMPT, /Composer 2\.5/, 'Composer 2.5 is a stale model name — use generic phrasing');
});

...
import { buildOctocodeRenderCall, buildOctocodeRenderResult, buildResultStats, buildToolCallSummary, makeRenderer, singleLineRenderer, truncateToWidth, visibleWidth, wrapText, } from '../src/tools/render-helpers.js';
const theme = {
    bold: (text) => `<b>${text}</b>`,
...
test('Octocode renderers cover partial, collapsed, expanded, stats, and error states', () => {
    ...
    assert.equal(buildOctocodeRenderResult('localSearchCode', textResult('still running'), { isPartial: true }, theme).render(120)[0], '<warning>localSearchCode running…</warning>');

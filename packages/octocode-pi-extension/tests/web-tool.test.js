...
async function loadRegisteredWebTool(out) {
    vi.resetModules();
    const runWebTool = vi.fn(async () => out);
    const renderWebResult = vi.fn((result) => {
        const r = result;
    ...
    const registerFn = (targetPi, names, def) => {
...
test('registerWebTool registers schema and executes through runWebTool', async () => {
    ...
    assert.ok(tool.parameters.properties?.['url']);
    ...
    const calls = runWebTool.mock.calls;
...
test('registerWebTool execute throws provider errors so Pi marks the call failed', async () => {
    ...
    await assert.rejects(() => tool.execute('call-1', { query: 'docs' }), /provider unavailable/);
});
...
test('web renderResult covers partial, search stats, page stats, expanded text, and errors', async () => {
    const { tool } = await loadRegisteredWebTool({});
    assert.equal(tool.renderResult(textResult('pending'), { isPartial: true }, theme).render(80)[0], '<warning>Fetching…</warning>');
    ...
    const error = tool.renderResult(textResult('bad', {}, true), { expanded: false }, theme).render(120)[0];

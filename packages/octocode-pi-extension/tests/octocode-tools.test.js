...
test('registerOctocodeTools registers schemas, skips one bad schema, executes success/error paths, and detects collisions', async () => {
    const executeDirectTool = vi.fn(async (name) => {
        if (name === 'npmSearch') {
    ...
    vi.doMock('@octocodeai/octocode-tools-core/schema', () => ({
        ...
        getDirectToolCategory: (name) => name.startsWith('local') ? 'Local Code' : 'GitHub',
        getDirectToolDescription: (name) => (name === 'npmSearch'
            ? 'Package | Search npm packages with precise names.'
            : `${name} | Full description for ${name}.`),
        formatDirectToolSchemaText: (name) => {
            if (name === 'ghCloneRepo')
    ...
    const tools = new Map();
    await registerOctocodeTools({ registerTool: (def) => tools.set(def.name, def) }, Type, new Set());
    ...
    try {
        const readResult = await tools.get('localGetFileContent').execute('call-read', { queries: [{ path: readable }, {}, { path: '   ' }] }, undefined, undefined, { cwd: tmp });
    ...
    aborted.abort();
    await assert.rejects(() => tools.get('localViewStructure').execute('call-2', { queries: [] }, aborted.signal), /cancelled before it started/);
    await assert.rejects(() => tools.get('npmSearch').execute('call-3', { queries: [] }), /package provider failed/);
    const names = new Set();
    ...
    assert.throws(() => registerUniqueTool({}, names, {
        name: 'duplicate',
        label: 'Duplicate',
        description: 'Duplicate test tool',
        parameters: Type.Object({}),

...
import { diffOps, diffOpsJs, generateDiffArtifacts, } from '../src/tools/edit-tool.js';
...
test('diffOps: append and delete reconstruct both sides', () => {
    ...
    assert.deepEqual(appendOps.filter((op) => op.type !== 'same'), [{ type: 'add', line: 'c' }]);
    ...
    assert.deepEqual(deleteOps.filter((op) => op.type !== 'same'), [{ type: 'remove', line: 'b' }]);
...
test('PERF: 3000-line single-change Myers diff stays under 5ms', () => {
    ...
    assert.ok(med < 5, `expected median Myers+artifacts < 5ms at 3k lines, got ${med.toFixed(2)}ms (was ~230ms+ LCS)`);
});
test('diffOpsJs and native engine agree on changed lines when native is available', () => {
    ...
    try {
        ...
        if (previous === undefined)
            delete process.env['OCTOCODE_EDIT_NATIVE_DIFF'];
        else
            process.env['OCTOCODE_EDIT_NATIVE_DIFF'] = previous;
    }
});

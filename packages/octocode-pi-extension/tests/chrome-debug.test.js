...
import { captureScreenshot, cleanupConnection, connectToChrome, createCdpSession, getSessionDir, getTargets, getVersion, inferIdentity, redactEvidence, redactObject, readSessionMeta, restrictedFetch, selectTarget, isLocalhost, buildScreenshotFilename, getPortUserDataDir, getScreenshotDir, getDefaultToolUserDataDir, buildRetryMarker, isCdpError, writeSessionMeta, } from '../src/chrome-debug.js';
import { SCHEME_REGISTRY, SCHEMES, ACTIONS, } from '../src/chrome-debug-schemes.js';
...
function runtimeResponseFor(expression) {
    if (expression === 'location.href')
        return runtimeValue('https://app.example/dashboard');
...
class MockCdpSession {
    ...
    emit(event, params, meta = {}) {
        for (const handler of this.handlers.get(event) ?? [])
            handler(params, meta);
        for (const handler of this.handlers.get('*') ?? [])
            handler(params, meta);
    }
    autoEmit(event, handler) {
        const emit = (params, meta = {}) => {
...
async function withLocalCdpServer(handler, fn) {
...
describe('SCHEME_REGISTRY', () => {
    test('every Scheme constant has an entry in SCHEME_REGISTRY', () => {
        for (const scheme of SCHEMES) {
            assert.ok(scheme in SCHEME_REGISTRY, `SCHEME_REGISTRY missing entry for scheme: "${scheme}"`);
        }
    });
    ...
    test('all schemes are fully implemented — no stubs remain', async () => {
        ...
        const FORMERLY_STUBBED = ['memory', 'css-coverage', 'js-coverage', 'websocket', 'emulate',
            ...
            'scrape', 'login', 'inject', 'monitor', 'full-audit'];
    ...
    test('raw scheme throws when method is missing', async () => {
        ...
        await assert.rejects(() => SCHEME_REGISTRY['raw'].recipe({
            session: fakeSession,
            params: { scheme: 'raw' },
            screenshotDir: os.tmpdir(),
            signal: undefined,
    ...
    test('raw scheme auto-enables domains and reads scriptFile through the path guard', async () => {
        ...
        try {
            ...
            assert.deepEqual(session.calls.slice(0, 2).map((call) => call.method), ['Page.enable', 'Page.addScriptToEvaluateOnNewDocument']);
...
test('ACTIONS includes all expected action verbs', () => {
    ...
    for (const a of expected) {
        assert.ok(ACTIONS.includes(a), `ACTIONS missing: "${a}"`);
    }
});
// ─── redactEvidence ───────────────────────────────────────────────────────────
...
describe('restrictedFetch and CDP HTTP sandbox', () => {
    test('restrictedFetch blocks non-localhost URLs before fetch runs', async () => {
        assert.throws(() => restrictedFetch('https://example.com/json/version'), /only localhost allowed/);
    });
...
test('getDefaultToolUserDataDir returns non-default profile path', () => {
    ...
    for (const def of osDefaults) {
        assert.ok(path.resolve(dir) !== path.resolve(def), `Tool userDataDir must not match OS default: ${def}`);
    }
});
...
describe('session metadata and identity', () => {
    ...
    test('inferIdentity captures page identity and cookie names only', async () => {
        const identity = await inferIdentity(new MockCdpSession(), 'attached', { Browser: 'Chrome/150', 'Protocol-Version': '1.3', 'User-Agent': 'Chrome' });
...
describe('native CDP session wrapper', () => {
    ...
    test('createCdpSession sends commands, routes events, and rejects after close', async () => {
        ...
        try {
            const session = await createCdpSession('ws://127.0.0.1/devtools/page/page-1', makeTarget(), 1000);
    ...
    test('createCdpSession rejects unavailable, aborted, errored, and timed-out sessions', async () => {
        ...
        try {
            ...
            await assert.rejects(() => createCdpSession('ws://127.0.0.1/devtools/page/page-1', makeTarget(), 100), /Native WebSocket not available/);
            const ac = new AbortController();
            ac.abort();
            globalThis.WebSocket = FakeWebSocket;
            await assert.rejects(() => createCdpSession('ws://127.0.0.1/devtools/page/page-1', makeTarget(), 100, ac.signal), /aborted/);
            ...
            await assert.rejects(() => createCdpSession('ws://127.0.0.1/devtools/page/page-1', makeTarget(), 100), /socket boom/);
            ...
            await assert.rejects(() => session.send('Runtime.evaluate'), /CDP timeout/);
            session.close();
        }
        finally {
            globalThis.WebSocket = previous;
...
test('connectToChrome attaches through fake CDP HTTP and WebSocket, writes metadata, and logs CDP events', async () => {
    ...
    globalThis.WebSocket = ConnectWebSocket;
    try {
        await withLocalCdpServer((req, res) => {
            res.setHeader('content-type', 'application/json');
            if (req.url === '/json/version') {
        ...
        if (previousDebug === undefined)
            delete process.env['OCTOCODE_CDP_DEBUG'];
        else
            process.env['OCTOCODE_CDP_DEBUG'] = previousDebug;
        fs.rmSync(tmp, { recursive: true, force: true });
    }
});
test('connectToChrome reports actionable guidance when Chrome is not responding', async () => {
    await withLocalCdpServer((_req, res) => {
        ...
        await assert.rejects(() => connectToChrome({ port, launch: false, timeoutMs: 100 }), /Chrome not responding on port/);
    });
});
test('cleanupConnection closes opened tabs and only kills launched browser processes when requested', async () => {
    ...
    try {
        await withLocalCdpServer((req, res) => {
            res.setHeader('content-type', 'application/json');
            if (req.url === '/json/close/opened-tab')
...
test('OCTOCODE_SUPPORT_TOOL_NAMES includes "chromeDebug"', () => {
    ...
    assert.ok(names.includes('chromeDebug'), `OCTOCODE_SUPPORT_TOOL_NAMES should include "chromeDebug". Got: ${names.join(', ')}`);
});
test('chromeDebug tool rejects unknown schemes and renders call/result states', async () => {
    const tools = new Map();
    registerChromeDebugTool({ registerTool: (def) => tools.set(def.name, def) }, Type, new Set(), (pi, names, def) => {
        names.add(def.name);
        pi.registerTool?.(def);
    });
    const tool = tools.get('chromeDebug');
    assert.ok(tool.parameters);
    await assert.rejects(() => tool.execute('call-1', { scheme: 'definitely-not-real' }), /Unknown scheme/);
    const themed = {
        bold: (text) => `<b>${text}</b>`,
    ...
    assert.equal(tool.renderResult(textToolResult('pending'), { isPartial: true }, themed).render(120)[0], '<warning>⧗ Connecting to Chrome…</warning>');
    const findingResult = textToolResult('[SESSION] attached\n[FINDING] bad cookie\n[ACTION] fix cookie\n[FINDING] mixed content', { scheme: 'security' });
    ...
    const screenshot = tool.renderResult(textToolResult('[SESSION] attached', { scheme: 'screenshot', screenshotPath: '/tmp/page-shot.png' }), { expanded: false }, themed).render(160)[0];
    assert.match(screenshot, /page-shot\.png/);
    const expanded = tool.renderResult(textToolResult(Array.from({ length: 35 }, (_, i) => (i % 2 ? `[ACTION] do ${i}` : `[FINDING] issue ${i}`)).join('\n'), { scheme: 'debug' }), { expanded: true }, themed).render(160);
    assert.equal(expanded.length, 32);
    assert.match(expanded.at(-1), /5 more lines/);
    const errored = tool.renderResult(textToolResult('bad', {}, true), { expanded: false }, themed).render(160)[0];
    assert.match(errored, /<error>✗<\/error>/);
});
function textToolResult(text, details = {}, isError = false) {
    return {
        isError,
...
test('chromeDebug tool execute path connects, runs a recipe, cleans up, redacts details, and renders', async () => {
    ...
    vi.doMock('../src/chrome-debug.js', () => ({
        ...
        cleanupConnection: async (_session, keepTab, killLaunched) => {
            cleanupCalls.push({ keepTab, killLaunched });
        },
        redactObject: (value) => {
            const copy = { ...value };
            if ('cookie_value' in copy)
                copy['cookie_value'] = '<redacted>';
            return copy;
        },
    }));
    ...
    try {
        ...
        let tool;
        registerChromeDebugTool({ registerTool: (def) => { tool = def; } }, Type, new Set(), registerUniqueTool);
        assert.ok(tool, 'tool registered');
        const registeredTool = tool;
        const statuses = [];
        const result = await registeredTool.execute('call-1', { scheme: 'debug', keepTab: false, port: 19333 }, undefined, undefined, { ui: { setStatus: (name, value) => statuses.push([name, value]) } });
        ...
        assert.equal(result.details['cookie_value'], '<redacted>');
        assert.equal(statuses.at(-1)?.[1], undefined, 'status should be cleared after success');
        const stealthResult = await registeredTool.execute('call-2', { scheme: 'debug', stealth: true, cleanup: true, port: 19333 }, undefined, undefined, { ui: { setStatus: (name, value) => statuses.push([name, value]) } });
        ...
        await assert.rejects(() => registeredTool.execute('call-3', { scheme: 'debug', action: 'fail', keepTab: true, port: 19333 }, undefined, undefined, { ui: { setStatus: (name, value) => statuses.push([name, value]) } }), /recipe exploded/);
        ...
        assert.match(registeredTool.renderResult({ content: [{ type: 'text', text: result.content[0].text }], details: result.details }, { expanded: false }).render(120)[0], /chromeDebug/);
    }
    finally {
...
test('chromeDebug tool is registered with scheme enum including "raw"', async () => {
    ...
    assert.ok(Array.isArray(props['scheme'].enum) && props['scheme'].enum.includes('raw'), 'scheme enum should include "raw"');
    assert.ok(Array.isArray(props['scheme'].enum) && props['scheme'].enum.includes('debug'), 'scheme enum should include "debug"');
    assert.ok(Array.isArray(props['scheme'].enum) && props['scheme'].enum.includes('screenshot'), 'scheme enum should include "screenshot"');
});
...
test('OCTOCODE_CHROME_DEBUG=0 prevents chromeDebug registration', async () => {
    ...
    try {
        ...
        if (prev === undefined)
            ...
            process.env['OCTOCODE_CHROME_DEBUG'] = prev;
    }
});
        ...
        try {
            ...
            try {
                const result = await SCHEME_REGISTRY['screenshot'].recipe({
                    session: conn.session,
                    params: { scheme: 'screenshot', format: 'png' },
                    screenshotDir: conn.screenshotDir,

...
test('extractBashWriteTargets finds redirects and tee', () => {
    ...
    assert.ok(extractBashWriteTargets('printf x | tee nested/a.txt', cwd).includes(path.join(cwd, 'nested/a.txt')));
});
...
test('bash override blocks writes outside allowed roots', async () => {
    ...
    try {
        await assert.rejects(() => bash.execute('1', { command: `echo pwned > /usr/octocode-bash-block-${process.pid}.txt` }, undefined, undefined, { cwd: tmp }), /bash write blocked|outside the allowed roots/);
        const ok = await bash.execute('2', { command: 'echo hello > ok.txt && cat ok.txt' }, undefined, undefined, { cwd: tmp });

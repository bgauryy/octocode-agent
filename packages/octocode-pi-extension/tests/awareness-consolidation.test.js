...
test('awareness consolidation: skill, CLI, hooks, and env are the only surfaces', async () => {
    ...
    const pi = {
        ...
        on: (e, h) => { const a = handlers.get(e) ?? []; a.push(h); handlers.set(e, a); },
    };
    const ext = (await import('../src/index.js')).default;
    await ext(pi);
    ...
    try {
        ...
        const cliCommands = JSON.parse(smoke.stdout).commands;
        ...
        if (previousAgentId == null)
            ...
            process.env.OCTOCODE_AGENT_ID = previousAgentId;
    }
});

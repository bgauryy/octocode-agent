...
async function registerBrowserAgentWithMocks(options = {}) {
    ...
    const tools = new Map();
    const pi = { registerTool: (def) => tools.set(def.name, def) };
    registerBrowserAgentTool(pi, Type, new Set(), (targetPi, names, def) => {
        names.add(def.name);
...
test('browserAgent runNow navigates, runs routed schemes, builds spawn config, and cleans up launched Chrome', async () => {
    const { tool, session, connectToChrome, cleanupConnection, securityRecipe, networkRecipe, } = await registerBrowserAgentWithMocks();
    ...
    const connectCalls = connectToChrome.mock.calls;
    const cleanupCalls = cleanupConnection.mock.calls;
    ...
    assert.match(String(tool.parameters.properties.model?.description ?? ''), /pi -ne --list-models/);

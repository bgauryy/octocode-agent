...
import octocodeDefault, { bundledAwarenessSkillRoot, createAwarenessHooksAddon, createOctocodePiExtension, resolvePromptMode, composeSystemPrompt, } from '../src/index.js';
...
test('createAwarenessHooksAddon wires the bundled skillRoot so the Pi harness gate is not a no-op', async () => {
    const root = bundledAwarenessSkillRoot();
    if (!root)
        return; // no bundle in this checkout — nothing to protect, gate correctly stays off
    ...
    try {
        ...
        const decision = await toolCall({ toolName: 'edit', toolCallId: 'harness-gate-1', input: { path: `${root}/SKILL.md` } }, { cwd: '/tmp' });
        assert.ok(decision && decision.block === true, 'editing a bundled skill file with OCTOCODE_ALLOW_HARNESS_APPLY unset must be blocked');
        assert.match(String(decision.reason), /editing the skill itself is gated/);
    }
    finally {
        if (previousAllow === undefined)
            delete process.env['OCTOCODE_ALLOW_HARNESS_APPLY'];
        else
            process.env['OCTOCODE_ALLOW_HARNESS_APPLY'] = previousAllow;
    }
});
test('resolvePromptMode: explicit option wins, then env, then append default', () => {
    ...
    try {
        ...
        if (previous === undefined)
            ...
            process.env['OCTOCODE_PROMPT_MODE'] = previous;
    }
});
test('composeSystemPrompt: append keeps Pi prompt first, octocode-first leads with harness', () => {
    ...
    assert.ok(octocodeFirst.indexOf('OCTO_HARNESS') < octocodeFirst.indexOf('PI_BASE'), 'octocode-first: harness leads');

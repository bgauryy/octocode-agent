...
import { isBlockedIp, assertPublicUrl, safeFetch, htmlToText, extractTitle, decodeEntities, unwrapDdgHref, parseDuckDuckGo, runWebTool, renderWebResult, pickProvider, normalizeApiKey, tavilySearch, serperSearch, resolveUserAgent, DEFAULT_USER_AGENT, DEFAULT_SEC_CH_UA, readCapped, createDeadline, postJson, webFetch, duckDuckGoSearch, webSearch, } from '../src/web.js';
// ── test helpers ──────────────────────────────────────────────────────────────
const publicLookup = async (_h) => [{ address: '8.8.8.8', family: 4 }];
const textRes = (body, opts = {}) => ({
...
test('assertPublicUrl rejects non-http(s) and hosts resolving to private IPs (DNS rebinding)', async () => {
    ...
    await assert.rejects(() => assertPublicUrl('http://attacker.com/', { lookup: async () => [{ address: '127.0.0.1', family: 4 }] }), /blocked address/);
...
test('safeFetch re-validates every redirect hop and blocks a redirect into a private IP', async () => {
    ...
    await assert.rejects(() => safeFetch('https://evil.com/', { fetchImpl: fetchImpl, lookup, timeoutMs: 1000 }), /private\/loopback|blocked address/);
});
...
test('webSearch: dispatches to the provider chosen by env, end to end', async () => {
    const tav = await webSearch('q', {
        ...
        fetchImpl: async () => jsonRes({ answer: 'AA', results: [{ title: 'T', url: 'u', content: 'c' }] }),
    });
    assert.equal(tav.engine, 'tavily');
    assert.equal(tav.answer, 'AA');
    const ser = await webSearch('q', {
        env: { SERPER_API_KEY: 'k' },
        fetchImpl: async () => jsonRes({ organic: [{ title: 'O', link: 'https://o', snippet: 's' }] }),
    });
    ...
    const ddg = await webSearch('q', {
        ...
        lookup: publicLookup,
        maxResults: 1,
        fetchImpl: async () => textRes('<a class="result__a" href="https://z.com">Z</a>'),
    });
...
test('safeFetch sends a browser-like User-Agent (overridable via env)', async () => {
    ...
    const fetchImpl = async (_url, init) => {
        hdrs = init?.headers;
        return { status: 200, ok: true, headers: new Map(), body: null, text: async () => '' };
    };
    const lookup = async () => [{ address: '8.8.8.8', family: 4 }];
...
test('runWebTool dispatches query options through webSearch', async () => {
    ...
    assert.equal(out.engine, 'duckduckgo');
    assert.equal(out.results?.[0]?.title, 'Docs');
});
